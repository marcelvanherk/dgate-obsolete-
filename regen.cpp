...
bcb 20120703: Changed WIN32 to !UNIX
bcb 20120710: Add #include for dgate split up
bcb 20121010: Added exclude hidden files
bcb 20130226: Replaced gpps with IniValue class, fixed size and strlen warnings.  Version to 1.4.18a.
*/
...
BOOL IsDirectory(char *path)
	...
	size_t lc= strlen(path) - 1;			/* index for last char in path */
	strcpy(temp, path);
...
RegenToDatabase(Database *DB, char *basename, char *filename, const char *device)
	...
	if (len < 4) return FALSE;
            ...
            if (stricmp(filename + len - 9, ".DS_Store"    ) == 0) return FALSE;
#endif //DARWIN
	...
	if (stricmp(filename + len - 3, ".v2")==0 )
		...
		if(!pDDO)
			{
			OperatorConsole.printf("***[Regen] %s -FAILED: Error on Load of v2 file\n", filename);
			return ( FALSE );
			}
		}
	else if ((stricmp(filename + len - 4, ".dcm")==0 ||
	         stricmp(filename + len - 4, ".img")==0 ||
	         strchr(filename + len - 4, '.')==NULL) && filename[0] != '.' //hiden
             )
		{
...
#ifdef DARWIN //A Macintosh OS X file.
				&& stricmp(FileData.cFileName, ".DS_Store"    ) != 0
#endif //DARWIN
		  		)
				{
				strcpy(TempPath, PathString);
...
Regen(const char *Device, int IsCacheOrJukeBox, char *SubDir)
	...
	GetPhysicalDevice(Device, PathString);
	itemp = strnlenint(PathString, sizeof(PathString));
	if(!itemp)
		return FALSE;
	...
	while ( TRUE )
		...
		if (strcmp(diren->d_name, ".") != 0 &&
			...
			if(strnicmp(Device, "JUKEBOX", 7)==0)
				{
				CDNumber = 0;
				for (i=strnlenint(FileString, sizeof(FileString))-1; i>=0; i--)
					{
					if (FileString[i] < '0' || FileString[i] > '9')
						{
						CDNumber = atoi(FileString + i + 1);
...
//		}
	...
	Index = 0;
	aSize = 0;
	if(iniValuePtr != NULL && iniValuePtr->sscscpPtr->JUKEBOXDevicePtr != NULL )
...
{ char data[1512];
  ...
  { IniValue *iniValuePtr = IniValue::GetInstance();
    ...
    { q[i] = new_queue(50, 1512, 10, regenprocess);
      DBs[i] = new Database;
      if (!DBs[i]->Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
                         iniValuePtr->sscscpPtr->Password, iniValuePtr->sscscpPtr->DataHost ) )
      { fprintf(stderr, "Error Connecting to SQL\n");
        return;
      }
