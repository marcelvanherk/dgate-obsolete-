...
BOOL	QueryOnStudy (
	...
	while ( Index < SQLResultStudy.GetSize() )
		...
		if(BuildSearchString(&DB, StudyDB, iniValuePtr->sscscpPtr->StudyTableName, EMaskStudy.Get(Index),
			SQLResultString, TempString, sizeof(TempString)-512))
			{
			if(CIndex++)
				safestrcat(SearchString, " and ", sizeof(SearchString));
		...
		BuildColumnString(StudyDB, iniValuePtr->sscscpPtr->StudyTableName, EMaskStudy.Get(Index), TempString);
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, TempString);
	...
	if (SQLResultPatient.GetSize())
		...
		sprintf(TempString, "%s.%s = %s.%s",
			iniValuePtr->sscscpPtr->StudyTableName, StudyDB [ LastDBE(StudyDB) ].SQLColumn,
			iniValuePtr->sscscpPtr->PatientTableName, StudyDB [ LastDBE(StudyDB) ].SQLColumn);
		safestrcat(SearchString, TempString, sizeof(SearchString));
		}

        if (iniValuePtr->sscscpPtr->StudyQuerySortOrder[0]) 
		{
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, iniValuePtr->sscscpPtr->StudyQuerySortOrder);
		}
	...
	if(DBEntryPatientName)
        ...
        Sort = Sorting;
        if (iniValuePtr->sscscpPtr->StudyQuerySortOrder[0]) Sort = iniValuePtr->sscscpPtr->StudyQuerySortOrder;
...
BOOL	QueryOnSeries (
	...
	while ( Index < SQLResultSeries.GetSize() )
		...
		if(BuildSearchString(&DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, EMaskSeries.Get(Index),
			SQLResultString, TempString, sizeof(TempString)-512))
			{
			if(CIndex++)
				safestrcat(SearchString, " and ", sizeof(SearchString));
		...
		BuildColumnString(SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, EMaskSeries.Get(Index), TempString);
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, TempString);
	...
	if (SQLResultStudy.GetSize() || SQLResultPatient.GetSize())
		...
		sprintf(TempString, "%s.%s = %s.%s",
			iniValuePtr->sscscpPtr->SeriesTableName, SeriesDB [ LastDBE(SeriesDB) ].SQLColumn,
			iniValuePtr->sscscpPtr->StudyTableName, SeriesDB [ LastDBE(SeriesDB) ].SQLColumn);
		safestrcat(SearchString, TempString, sizeof(SearchString));
		}

        if (iniValuePtr->sscscpPtr->SeriesQuerySortOrder[0]) 
		{
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, iniValuePtr->sscscpPtr->SeriesQuerySortOrder);
		}

	if (SQLResultPatient.GetSize())
	  sprintf(Tables, "%s, %s, %s",
	  	iniValuePtr->sscscpPtr->SeriesTableName, iniValuePtr->sscscpPtr->StudyTableName,
		iniValuePtr->sscscpPtr->PatientTableName);
	...
	// Issue Query
        ...
        if (iniValuePtr->sscscpPtr->SeriesQuerySortOrder[0]) Sort = iniValuePtr->sscscpPtr->SeriesQuerySortOrder;
...
// will be fine at these levels (allows multiplicity of 59)
	...
	while ( Index < SQLResultImage.GetSize() )
		...
		if(BuildSearchString(&DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, EMaskImage.Get(Index),
		...
		BuildColumnString(ImageDB, iniValuePtr->sscscpPtr->ImageTableName, EMaskImage.Get(Index), TempString);
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, TempString);
	...
	if (SQLResultSeries.GetSize() || SQLResultStudy.GetSize() || SQLResultPatient.GetSize())
		...
		sprintf(TempString, "%s.%s = %s.%s",
			iniValuePtr->sscscpPtr->ImageTableName, ImageDB [ LastDBE(ImageDB) ].SQLColumn,
			iniValuePtr->sscscpPtr->SeriesTableName, ImageDB [ LastDBE(ImageDB) ].SQLColumn);
		safestrcat(SearchString, TempString, MAXLENGTH);
		}
	...
	if (SendObjectFile)
		{
		strcat(ColumnString, ",");
		strcat(ColumnString, iniValuePtr->sscscpPtr->ImageTableName);
	...
	if (iniValuePtr->sscscpPtr->ImageQuerySortOrder[0]) 
		{
		if(CCIndex)
			strcat(ColumnString, ", ");
		strcat(ColumnString, iniValuePtr->sscscpPtr->ImageQuerySortOrder);
		}
	...
	// Issue Query
        if (iniValuePtr->sscscpPtr->ImageQuerySortOrder[0]) Sort = iniValuePtr->sscscpPtr->ImageQuerySortOrder;
...
//		}
	...
	while ( Index < SQLResultWorkList.GetSize() )
		...
		if (SQLResultString)						// flat query - but sequence starters (NULL) are not queried
			{
			if(BuildSearchString(&DB, WorkListDB, iniValuePtr->sscscpPtr->WorkListTableName, EMaskWorkList.Get(Index),
				SQLResultString, TempString, sizeof(TempString)-512))
				{
				if(CIndex++)
					safestrcat(SearchString, " and ", sizeof(SearchString));
			...
			BuildColumnString(WorkListDB, iniValuePtr->sscscpPtr->WorkListTableName, EMaskWorkList.Get(Index), TempString);
			if(ColumnString[0])
				strcat(ColumnString, ", ");
			strcat(ColumnString, TempString);
			}
	...
	while (DB.NextRecord())
		...
		if (iniValuePtr->sscscpPtr->WorkListReturnsISO_IR_100)
			{
			vr = RDDO->GetVR(0x0008, 0x0005);
			if(!vr)
				{
				VR *vrNew = new VR(0x0008, 0x0005, 10, TRUE);
				sprintf((char*)vrNew->Data, "ISO_IR %d", iniValuePtr->sscscpPtr->WorkListReturnsISO_IR_100);
				RDDO->Push(vrNew);
				}
			}
