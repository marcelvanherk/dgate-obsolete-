...
20130331	bcb	Used strnlen32u to fix size warnings
*/
...
//		vr = new VR(0x0002, 0x0010, strlen(_LittleEndianUID), TRUE);
		vr = new VR(0x0002, 0x0010, strnlen32u(pszTransferSyntaxUID), TRUE);
...
//		memcpy(vr->Data, (void*)_LittleEndianUID, strlen(_LittleEndianUID));
		...
		vr->TypeCode = 'UI';
		DDOPtr->Push(vr);
		vr = new VR(0x0002, 0x0012, strnlen32u(_ImplementationUID), TRUE);
		memset(vr->Data, 0, vr->Length);
		memcpy(vr->Data, (void*)_ImplementationUID, strlen(_ImplementationUID));
		vr->TypeCode = 'UI';
		DDOPtr->Push(vr);
		vr = new VR(0x0002, 0x0013, strnlen32u(_ImplementationVersion), TRUE);
		memset(vr->Data, 0, vr->Length);
