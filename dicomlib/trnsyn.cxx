...
20120703   bcb   fixed datefix, removed semicolon from the end of an if,
                   moved SwapBigEndianData from here to deivr.
...
//bcb 20120703 belongs in deivr, moved there.
...
//                        			iOrgMode = 0; //No need to check again, we were told what it is. bcb 20100419
		...
		if ( vr->Length )
			...
			lVRBuffer.Read ( (BYTE *) vr->Data, vr->Length);
			if (Mode==TSA_EXPLICIT_BIG_ENDIAN)
				vr->SwapBigEndianData(); //SwapBigEndianData(vr);
