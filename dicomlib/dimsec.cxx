...
20130226	bcb	Replaced gpps with IniValue class, fixed strlen warnings.  Version to 1.4.18a.
20130313	bcb	Merged with 1.4.17a.
20130328	bcb	Removed ConfigPadAEWithZeros
*/
/****************************************************************************
...
BOOL	CStoreRQ	::	Write (
	...
	if (CallingAE)
		{
		VR	*vr2;
		UINT32     len = strnlen32u((char *)CallingAE,ACRNEMA_AE_LEN);
		if (len&1) len++;
...
//		DCOR.Push ( vr );
	...
	if(DDO)
		...
		if (!DDO->GetVR(0x0008, 0x0052))
			...
			UINT32	iLength;
			iLength = strnlen32u(QueryRetrieveLevel);
			if (iLength & 0x01)
				iLength++;
...
//	if (IniValue::GetInstance()->sscscpPtr->ConfigPadAEWithZeros) memset((char*) WhereACRNema, 0, 16); bcb 130328
