...
20120703	bcb	Removed WHEDGE, made dgatefn.hpp
20120710	bcb	Used ROOTCONFIG_SZ, fix includes
...
20130226	bcb Replaced gpps with IniValue class.  Removed all globals now in IniValue class.
                        Version to 1.4.18a.
20130328	bcb Merged with 1.4.17
*/
...
GetFileNameSyntax(unsigned int *maxfilenamelength)
	...
	if (FileNameSyntax<0)
		...
		if (strchr(iniValuePtr->sscscpPtr->FileNameSyntaxString, '%'))          FileNameSyntax=999;	// configured by string
		else if (memcmp(iniValuePtr->sscscpPtr->FileNameSyntaxString, "lua:", 4)==0)
			FileNameSyntax=998;	// lua done by caller (dcm assumed)
		else FileNameSyntax = atoi(iniValuePtr->sscscpPtr->FileNameSyntaxString);

		MaxLength = iniValuePtr->sscscpPtr->MaxFileNameLength;
		}
...
//	VR	*TempVR;
	...
	if (GetFileName ( DDOPtr->GetVR(0x0008, 0x0018),
		...
		*Root=0;
		...
		if (iFileNameSyntax==4 || iFileNameSyntax==998 || (iFileNameSyntax>=8 && iFileNameSyntax<=12)) 
			...
			q = strrchr(iniValuePtr->sscscpPtr->FileNameSyntaxString, '.');
	...
	switch (iFileNameSyntax)
		...
		strcpy(filename, iniValuePtr->sscscpPtr->FileNameSyntaxString);
		break;

		case 999:	// configured filename: any string with %, e.g., %studydate[0,3]\%name_%id\%sopuid.v2
                filename[0] = 0;
                i = 0;
                L = strlen(iniValuePtr->sscscpPtr->FileNameSyntaxString);
                while (i<L)
                { if (iniValuePtr->sscscpPtr->FileNameSyntaxString[i] == '%')
                  { char *ps = filename + strlen(filename);

                    i++;
                    p = iniValuePtr->sscscpPtr->FileNameSyntaxString + i;
		    ...
		    { i+=4;
		      time_t rawtime;
  	  	    ...
  	  	    // get substring of variable
                    if (iniValuePtr->sscscpPtr->FileNameSyntaxString[i]=='[')
                    { int a=0, b=-1;
                      sscanf(iniValuePtr->sscscpPtr->FileNameSyntaxString+i, "[%d,%d]", &a, &b);
                      for (;i<L;i++) if (iniValuePtr->sscscpPtr->FileNameSyntaxString[i-1]==']') break;
		    ...
		    // unrecognized names are not translated and appear as such in filename - good for debugging
                  ...
                  { l = strlen(filename);
                    filename[l]   = iniValuePtr->sscscpPtr->FileNameSyntaxString[i++];
                    filename[l+1] = 0;
                  }
                }
		break;
