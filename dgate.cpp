...
extern char	TransmitCompression[];			// compression of transmitted images
extern char	DefaultNKITransmitCompression[];		// default compression of transmitted images using NKI C-MOVE
//extern char	ArchiveCompression[];				// compression of images to be archived
extern char	TestMode[];				// if set, appended to filename for comparison
extern char	StatusString[];				// status of submission and move for web i/f
...
//	char szRootSC[ROOTCONFIG_SZ];
	...
	if (error==DCM_ERROR_DEBUG)
		...
		{
		if (IniValue::GetInstance()->sscscpPtr->IgnoreOutOfMemoryErrors)return;
...
{ char TimeString[80], buf[64];
  ...
  fprintf(f, "Activity: Echo:%d, Find:%d, Move:%d, Unknown:%d, gpps:%d\n", EchoRequest, C_Find_PatientRoot+C_Find_StudyRoot+C_Find_PatientStudyOnly+C_Find_ModalityWorkList,
  	C_Move_PatientRootNKI+C_Move_PatientRoot+C_Move_StudyRootNKI+C_Move_StudyRoot+C_Move_PatientStudyOnlyNKI+C_Move_PatientStudyOnly, UnknownRequest, iniValuePtr->gpps);
  ...
  if(iniValuePtr->sscscpPtr->MAGDevicePtr != NULL) aSize = iniValuePtr->sscscpPtr->MAGDevicePtr->GetSize();
  for (i=0; i<aSize; i++)
    fprintf (f, "Space on MAG%d  : %d MByte\n", i, CheckFreeStoreOnMAGDevice(i));
  aSize = 0;
  if(iniValuePtr != NULL && iniValuePtr->sscscpPtr->CACHEDevicePtr != NULL )
...
NonDestructiveDumpDICOMObject(
	DICOMObject	*DO, FILE *f, int iDepth)
	{
	DICOMObject	DO2;
	VR		*vr;
	int		i;
...
PrintFreeSpace()
	...
	Index = 0;
	aSize = 0;
	if(iniValuePtr != NULL && iniValuePtr->sscscpPtr->MIRRORDevicePtr != NULL )
...
//	UINT	Index = 0;
	...
	DumpDD(PatientDB);
	OperatorConsole.printf ("--Study Database: %s\n", iniValuePtr->sscscpPtr->StudyTableName);
	DumpDD(StudyDB);
	OperatorConsole.printf ("--Series Database: %s\n", iniValuePtr->sscscpPtr->SeriesTableName);
	DumpDD(SeriesDB);
	OperatorConsole.printf ("--Image Database: %s\n", iniValuePtr->sscscpPtr->ImageTableName);
	DumpDD(ImageDB);
	if (iniValuePtr->sscscpPtr->WorkListMode != 0)//bcb No worklist mode, why print.
		{
		OperatorConsole.printf ("--WorkList Database: %s\n", iniValuePtr->sscscpPtr->WorkListTableName);
		DumpDD(WorkListDB);
		}
	return ( TRUE );
	}
...
/* bcb 20120703 moved to hpp
 static BOOL DcmMove(const char *patid, char* pszSourceAE, char* pszDestinationAE, const char *studyuid, const char *seriesuid, const char *compress, const char *modality, const char *date, 
		...
		*ser++= 0;
		...
		if(!GetFileName(vr, Filename, Device, TRUE, NULL, stud, ser))
			...
			delete vr;
 			...
 			IniValue *iniValuePtr = IniValue::GetInstance();
			DcmMove("", iniValuePtr->sscscpPtr->MyACRNema,
                    iniValuePtr->sscscpPtr->MyACRNema, stud, ser, "", "", "", sop, "", "", 7, "");

			/*if(iniValuePtr != NULL && iniValuePtr->sscscpPtr->VirtualServerForPtr != NULL)
				{
				unsigned aSize = iniValuePtr->sscscpPtr->VirtualServerForPtr->GetSize();
				for (unsigned int i=0; i<aSize; i++)
					{ 
					char *q, tmp[64];
					strcpylim(tmp, iniValuePtr->sscscpPtr->VirtualServerForPtr->Get(i));
					q = strchr(tmp, ',');
					if (q) *q=0;
					if (!DcmMove("", tmp, iniValuePtr->sscscpPtr->MyACRNema,
					    stud, ser, "", "", "", sop, "", "", 7, "")) break;
					}
				}
			*/
...
// external reference from nkiqrsop.cxx bcb 20100703 moved to nkiqrsop.hpp
...
class	MyBridgeStorage	:	public UnknownStorage
	...
	public:
		...
		UINT16 CheckObject(DICOMDataObject *DDO, PDU_Service *PDU) 
			...
			IniValue *iniValuePtr = IniValue::GetInstance();
			if (!Compression) Compression = iniValuePtr->sscscpPtr->IncomingCompression; else Compression++;
...
//			char szTemp[MAX_PATH];
			if(iniValuePtr->sscscpPtr->TempDir[0] != 0)
				strcpy(Filename, iniValuePtr->sscscpPtr->TempDir);
			else
				{
				Filename[0]=0;
...
DICOMDataObject *LoadForBridge(char *stsesop, char *ae)
	...
	if (ser && sop)
		...
		*ser++= 0;
		...
		IniValue *iniValuePtr = IniValue::GetInstance();
		DcmMove("", ae, iniValuePtr->sscscpPtr->MyACRNema, stud, ser, "", "", "", sop, "", "", 0x2bad, "");
		if(iniValuePtr->sscscpPtr->TempDir[0] != 0)
		    strcpy(Filename, iniValuePtr->sscscpPtr->TempDir);
		else
			{
...
static BOOL
NewUIDsInDICOMObject(DICOMObject *DO, const char *Exceptions, const char *Reason)
	{
	DICOMObject	DO2;
	VR		*vr;
	int		TypeCode;//, i;
...
static BOOL
ModifyPATIDofDDO(DICOMDataObject *pDDO, char *NewPATID, char *Reason)
	{
	int			len;
	PDU_Service		PDU;
	DICOMDataObject		DDO;
...
//	VR			*vrSOPInstanceUID;
	...
	rc = recompress(&pDDO, iniValuePtr->sscscpPtr->DroppedFileCompression, "",
		iniValuePtr->sscscpPtr->DroppedFileCompression[0]=='n' ||
		iniValuePtr->sscscpPtr->DroppedFileCompression[0]=='N', PDU);
	...
	if ((rc == FALSE) || (!SaveToDisk(DB, pDDO, rFilename, TRUE, PDU, 0, !(iniValuePtr->sscscpPtr->ImportExportDragAndDrop))))
		{
...
GenUID(char	*oString)
	{ 
	static char UIDPrefix[66] = "1.2.826.0.1.3680043.2.135.1066.6"; // default value, only used here.
	static BOOL dgLoadRoot = TRUE; // default value, only used here.
	if (dgLoadRoot)
		{
		strcpy(UIDPrefix, IniValue::GetInstance()->sscscpPtr->UIDPrefix);
		strcat(UIDPrefix, ".6");
		dgLoadRoot = FALSE;
		}
...
int TestFilter(char *query, char *sop, int maxname, char *patid)
{	Database aDB;
...
//	char *p;
	...
	if (*query)
	 	...
	 	iniValuePtr->sscscpPtr->ImageTableName, iniValuePtr->sscscpPtr->SeriesTableName,
		iniValuePtr->sscscpPtr->StudyTableName, iniValuePtr->sscscpPtr->PatientTableName);
	else
	 	sprintf(Tables, "%s",
			iniValuePtr->sscscpPtr->ImageTableName);
	...
	if(!aDB.Open(iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
		...
		OperatorConsole.printf("***Filter: Unable to open database %s as user %s on %s\n",
			iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
...
{ char		szTemp[66];
  ...
  char		*szExecName, szNext[512];
  const char *szExecModality;//[66];
  const char *szExecStationName;//[66];
  char		szExecFilter[512];
  const char *ExportCalledAE, *ExportCallingAE;
  char		Called[18], Calling[18];
  BOOL		StripGroup2;
  const char *wild = "*\0";
  ...
  IniValue *iniValuePtr = IniValue::GetInstance();
  if(iniValuePtr->luaPtr != NULL && iniValuePtr->luaPtr->ExportConverterPtr != NULL
     && N < (int)iniValuePtr->luaPtr->ExportConverterPtr->GetSize()
     && (szExecName = iniValuePtr->luaPtr->ExportConverterPtr->Get(N)) != NULL)
//  if (szExecName[0])
  { if (!DDO) DDO = PDU2.LoadDICOMDataObject(pszFileName);
    struct scriptdata lsd1 = {PDU, NULL, DDO, N, pszModality, pszStationName, pszSop, patid, NULL, 0};
  ...
  if (pszModality)
  { if(iniValuePtr->sscscpPtr->ExportPtr->ModalityPtr != NULL
    && N < (int)iniValuePtr->sscscpPtr->ExportPtr->ModalityPtr->GetSize()
    && (szExecModality = iniValuePtr->sscscpPtr->ExportPtr->ModalityPtr->Get(N)) != NULL)
    {//All good! Check for match
      if (!match(pszModality, szExecModality)) return FALSE;
    }
    else szExecModality = wild;
  }

  if (pszStationName)
  { if(iniValuePtr->sscscpPtr->ExportPtr->StationNamePtr != NULL
    && N < (int)iniValuePtr->sscscpPtr->ExportPtr->StationNamePtr->GetSize()
    && (szExecStationName = iniValuePtr->sscscpPtr->ExportPtr->StationNamePtr->Get(N)) != NULL)
    {//All good! Check for match
      if (!match(pszStationName, szExecStationName)) return FALSE;
    }
    else szExecStationName = wild;
  }

  if (calling)
  { if(iniValuePtr->sscscpPtr->ExportPtr->CallingAEPtr != NULL
    && N < (int)iniValuePtr->sscscpPtr->ExportPtr->CallingAEPtr->GetSize()
    && (ExportCallingAE = iniValuePtr->sscscpPtr->ExportPtr->CallingAEPtr->Get(N)) != NULL)
    {//All good! Check for match
      if (!match(calling, ExportCallingAE)) return FALSE;
    }
    else ExportCallingAE = wild;
  }

  if (called)
  { if(iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr != NULL
    && N < (int)iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr->GetSize()
    && (ExportCalledAE = iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr->Get(N)) != NULL)
    {//All good! Check for match
      if (!match(called, ExportCalledAE)) return FALSE;
    }
    else ExportCalledAE = wild;
  }
  ...
  { szExecFilter[0]='*';
    ...
    if(iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr != NULL
        && N < (int)iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr->GetSize())
        strcpylim(szExecFilter, iniValuePtr->sscscpPtr->ExportPtr->CalledAEPtr->Get(N), 512);
    if (szExecFilter[0]!='*') 
    { if (iniValuePtr->sscscpPtr->TruncateFieldNamesLen == 0) return FALSE;//Found, but no value.
      if (iniValuePtr->sscscpPtr->TruncateFieldNamesLen == -1) MaxLength = 255;//Not found, other default.
      else MaxLength = iniValuePtr->sscscpPtr->TruncateFieldNames;
  ...
  if (called && calling)
  { if(iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr == NULL
    || N >= (int)iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr->GetSize()
    || (szExecName = iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr->Get(N)) == NULL) return FALSE;
...
//        char vr[256];
	...
	strcpy(line, szExecName);
      ...
      { char host[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN], dummy[ACRNEMA_COMP_LEN];//, Level[64], Temp[66];
        char *p, *q, *c;
	...
	// get the script clause (to process), must be last
        ...
        strcpy((char *)org, iniValuePtr->sscscpPtr->MyACRNema);
	...
	if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild(line+offset, (char *)host, (char *)port, c))
	  ...
	  strcpy(host, line+offset);
        ...
        vr = NULL;
        switch(iniValuePtr->sscscpPtr->Level)
        {
          case LT_PATIENT:
	...
	if (vr) szTemp[vr->Length]=0;
        ...
        { SetUID ( iUID, vr );
          ...
          { //OperatorConsole.printf("!!! ExportConverter%d.%d: attempt to reconnect %s \n", N, part, szTemp);
            if (iniValuePtr->sscscpPtr->ForwardAssociationRelease) PDU[part].Close();
	  ...
	  if (dest[0])
          ...
          PDU[part].SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);
	...
	// process data
        ...
        { OperatorConsole.printf("*** ExportConverter%d.%d: Forward failed to send DICOM image to %s\n", N, part, line+offset);
          PDU[part].Close();
          delete DDO;
          if (iniValuePtr->sscscpPtr->RetryForwardFailed) return TRUE;
	  else            return FALSE; // NOT a good idea to retry later
        }
      
        ImagesForwarded++;
        if (iniValuePtr->sscscpPtr->Level == LT_IMAGE)//(level==4) 
        { if (iniValuePtr->sscscpPtr->ForwardAssociationRelease) PDU[part].Close();
          else		  PDU[part].Link.Close();
        }
	...
	DDO=NULL;
	  ...
	  else                                      ToJPG(DDO, file, size, 0, level, window, frame);
        }
      }
	...
	for (int sIndex = 0; sIndex<=strlen(cline); sIndex++)
      ...
      { FILE *f;
        int ret;
        char cmd[1024], *cmdPtr;
        
	if (!DDO) DDO = PDU2.LoadDICOMDataObject(pszFileName);

        cmd[0]=0;
        if(iniValuePtr->scriptsPtr != NULL && (cmdPtr = iniValuePtr->GetIniTableValue(line+5, iniValuePtr->scriptsPtr)) != NULL)
        {
          strcpylim(cmd, cmdPtr, 1024);
...
{ char		szRootSC[ROOTCONFIG_SZ];
  ...
  if (!MyGetPrivateProfileString(RootConfig, "MicroPACS", RootConfig,
		szRootSC, ROOTCONFIG_SZ, ConfigFile)) return;
...
//  char		*szEntry;//[32];
  char		szTemp[66];
  char		szNext[512];
  char		szExecName[BUF_CMD_LEN];
//  char                  szExecModality[66];
//  char                  szExecStationName[66];
  char		Called[18], Calling[18];//ImportCalledAE[18], ImportCallingAE[18];
  int           rc, part, skipping;
  char		ininame[30];
  static 	ExtendedPDU_Service PDU2[20*20]; // for direct forwards, not script context
  static char 	ForwardUIDS[66 * 20*20];
  char		pszFileName[512];
  iEGroup      *iEGroupPtr, *luaIEGroupPtr;
  ...
  if (DDO==NULL)
  { if (N>=0 && N<20) for (int i=0; i<20; i++) PDU2[N*20+i].Close();
    return FALSE;
  }
/*
...
*/
  ...
  {
    case 1000:
      iEGroupPtr = iniValuePtr->sscscpPtr->QueryPtr;
      break;
    case 1100:
      iEGroupPtr = iniValuePtr->sscscpPtr->WorkListQueryPtr;
      break;
    case 1200:
      iEGroupPtr = iniValuePtr->sscscpPtr->RetrievePtr;
      break;
    case 1300:
      iEGroupPtr = iniValuePtr->sscscpPtr->RetrieveResultPtr;
      break;
    case 1500:
      iEGroupPtr = iniValuePtr->sscscpPtr->QueryResultPtr;
      break;
    case 1600:
      iEGroupPtr = iniValuePtr->sscscpPtr->ModalityWorkListQueryResultPtr;
      break;
    case 1700:
      iEGroupPtr = iniValuePtr->sscscpPtr->MergeSeriesPtr;
      break;
    case 1800:
      iEGroupPtr = iniValuePtr->sscscpPtr->MergeStudiesPtr;
      break;
    case 1900:
      iEGroupPtr = iniValuePtr->sscscpPtr->ArchivePtr;
      break;
    case 2000:
      iEGroupPtr = iniValuePtr->sscscpPtr->MoveDevicePtr;
      break;
    case 2100:
      iEGroupPtr = iniValuePtr->sscscpPtr->RejectedImagePtr;
      break;
    case 2200:
      iEGroupPtr = iniValuePtr->sscscpPtr->RejectedImageWorkListPtr;
      break;
    default:
      if ((N/100)==14) iEGroupPtr = iniValuePtr->sscscpPtr->CompressionPtr;
      else iEGroupPtr = iniValuePtr->sscscpPtr->ImportPtr;
      break;
  }
  if (N>=0)
  {
    if(iniValuePtr->luaPtr != NULL) switch (N)
    {
      case 1000:
        luaIEGroupPtr = iniValuePtr->luaPtr->QueryPtr;
        break;
      case 1100:
        luaIEGroupPtr = iniValuePtr->luaPtr->WorkListQueryPtr;
        break;
      case 1200:
        luaIEGroupPtr = iniValuePtr->luaPtr->RetrievePtr;
        break;
      case 1300:
        luaIEGroupPtr = iniValuePtr->luaPtr->RetrieveResultPtr;
        break;
      case 1500:
        luaIEGroupPtr = iniValuePtr->luaPtr->QueryResultPtr;
        break;
      case 1600:
        luaIEGroupPtr = iniValuePtr->luaPtr->ModalityWorkListQueryResultPtr;
        break;
      case 1700:
        luaIEGroupPtr = iniValuePtr->luaPtr->MergeSeriesPtr;
        break;
      case 1800:
        luaIEGroupPtr = iniValuePtr->luaPtr->MergeStudiesPtr;
        break;
      case 1900:
        luaIEGroupPtr = iniValuePtr->luaPtr->ArchivePtr;
        break;
      case 2000:
        luaIEGroupPtr = iniValuePtr->luaPtr->MoveDevicePtr;
        break;
      case 2100:
        luaIEGroupPtr = iniValuePtr->luaPtr->RejectedImagePtr;
        break;
      case 2200:
        luaIEGroupPtr = iniValuePtr->luaPtr->RejectedImageWorkListPtr;
        break;
      default:
        if ((N/100)==14) luaIEGroupPtr = iniValuePtr->luaPtr->CompressionPtr;
        else luaIEGroupPtr = iniValuePtr->luaPtr->ImportPtr;
        break;
    }
    N = N%100;
    ...
    if(pszModality && iEGroupPtr != NULL && iEGroupPtr->ModalityPtr != NULL &&
      (unsigned int)N < iEGroupPtr->ModalityPtr->GetSize())
        if (!match(pszModality, iEGroupPtr->ModalityPtr->Get(N))) return FALSE;

    if(pszStationName && iEGroupPtr != NULL && iEGroupPtr->StationNamePtr != NULL &&
      (unsigned int)N < iEGroupPtr->StationNamePtr->GetSize())
        if (!match(pszModality, iEGroupPtr->StationNamePtr->Get(N))) return FALSE;

    if(calling && iEGroupPtr != NULL && iEGroupPtr->CallingAEPtr != NULL &&
      (unsigned int)N < iEGroupPtr->CallingAEPtr->GetSize())
        if (!match(pszModality, iEGroupPtr->CallingAEPtr->Get(N))) return FALSE;

    if(called && iEGroupPtr != NULL && iEGroupPtr->CalledAEPtr != NULL &&
      (unsigned int)N < iEGroupPtr->CalledAEPtr->GetSize())
        if (!match(pszModality, iEGroupPtr->CalledAEPtr->Get(N))) return FALSE;

    /* get the import converter name */
          
    if(called && calling && iEGroupPtr != NULL && iEGroupPtr->ConverterPtr != NULL &&
      (unsigned int)N < iEGroupPtr->ConverterPtr->GetSize())
        strcpy(szExecName, iEGroupPtr->ConverterPtr->Get(N));
  }
  if(szExecName[0] == 0) strcpylim(szExecName, Script, BUF_CMD_LEN);
  ...
  { char line[1000], vr1[256];
    ...
    { if      (i==0 && szExecName[i] == '{') {i++; while(szExecName[i]==' ')i++;}
	  ...
	  case 'q': level1 = szExecName[i+2];	// %QXxxxx,yyyy=from db (group and element must have 4 digits)
			...
			break;
                      case 'w'://bcb? Should we test worklist mode != 0 here.
    ...
    { char host[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN], dummy[ACRNEMA_COMP_LEN];//, Level[64], Temp[66];
      ...
      UID			iUID, uid;
      VR			*vr;
      int			offset = 11, channel;//, level;
      DICOMDataObject		*pDDO;
      BOOL		        StripGroup2;
      BYTE			org[512], dest[512];
      char			script[512];
      ExtendedPDU_Service	*lPDU;

      if (N<0 || N>=20) N = 19;  // typically from lua
      lPDU = &PDU2[N*20];
      char *ForwardLastUID = ForwardUIDS + 66 * 20 * N;
      pDDO = MakeCopy(DDO);
      ...
      if (p)
      { channel = atoi(p+9);
        if (channel<0 || channel>=20) channel = 0;
      }
	...
	strcpy(compress, "UN");
      ...
      vr = NULL;
/*      level = 0;
      MyGetPrivateProfileString(RootConfig, "MicroPACS", RootConfig, szRootSC, ROOTCONFIG_SZ, ConfigFile);
      ...
      else if (strncmp(Level, "SOPCLASS",8)==0) level=5, lPDU[channel].ClearAbstractSyntaxs();
      ...
      // does the existing link accept images of this type - if not then hang up to force a reconnect
      // also hang up when UID at ForwardAssociationLevel changes to start a new association
      if (lPDU[channel].Link.Connected)
      { SetUID ( iUID, vr );
        if (!lPDU[channel].IsAbstractSyntaxAccepted(iUID) || strcmp(ForwardLastUID+channel*66, szTemp)!=0 )
        { //OperatorConsole.printf("!!! ExportConverter%d.%d: attempt to reconnect %s \n", N, channel, szTemp);
          if (iniValuePtr->sscscpPtr->ForwardAssociationRelease) lPDU[channel].Close();
	  else            lPDU[channel].Link.Close();
        }
      }
      ...
      // (re)connect
      if (!lPDU[channel].Link.Connected)
      { lPDU[channel].AttachRTC(&VRType);
        lPDU[channel].SetRequestedCompressionType(compress);

        lPDU[channel].SetApplicationContext ( AppUID );
        lPDU[channel].SetLocalAddress ( org );
	if (dest[0])
          lPDU[channel].SetRemoteAddress ( (unsigned char *)(dest) );
	else
          lPDU[channel].SetRemoteAddress ( (unsigned char *)(line+offset) );
        lPDU[channel].SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);

        vr = pDDO -> GetVR(0x0008, 0x0016);
        SetUID ( iUID, vr );
        lPDU[channel].AddAbstractSyntax ( iUID );		// adds type of this image to presentation contexts
  
        uid.Set("1.2.840.10008.1.1");		// This one should always accept: verification
        lPDU[channel].AddAbstractSyntax(uid);		// assures connect will not return FALSE because image is not accepted
  
        if (!lPDU[channel].Connect((unsigned char *)&host, (unsigned char *)&port))
        { OperatorConsole.printf("*** %sConverter%d.%d: Forward failed to connect to host %s\n", ininame, N, channel, line+offset);
        }
	else
	{ vr = pDDO -> GetVR(0x0008, 0x0016);
	  SetUID ( iUID, vr );
	  if (!lPDU[channel].IsAbstractSyntaxAccepted(iUID))
      ...
      { // process data
        if (script[0])
          CallImportConverterN(pDDO, -1, NULL, NULL, NULL, NULL, lPDU, NULL, script);
        
        // recompress data to be forwarded here according to accepted compression mode; strip group 2 unless "as" or "is"
	p = lPDU[channel].GetAcceptedCompressionType(iUID);
	StripGroup2 = memicmp(p, "as", 2)!=0 && memicmp(p, "is", 2)!=0;
	recompress(&pDDO, p, "", StripGroup2, lPDU);

	RTCStorage.SetUID(iUID);

	if (!RTCStorage.Write(lPDU+channel, pDDO))
	{ OperatorConsole.printf("*** %sConverter%d.%d: Forward failed to send DICOM image to %s\n", ininame, N, channel, line+offset);
  	  lPDU[channel].Close();
	}
	else
	{ ImagesForwarded++;
	  if (iniValuePtr->sscscpPtr->Level == LT_IMAGE) 
	  { if (iniValuePtr->sscscpPtr->ForwardAssociationRelease) lPDU[channel].Close();
	    else	    lPDU[channel].Link.Close();
	  }
	}
    ...
    { char cline[512];
      ...
      DO2 = llPDU.LoadDICOMDataObject(tempfile);
      while (( pVR = DO2->Pop() ))
      { DDO->Push(pVR);
      }
      delete DO2;
    ...
    { FILE *f;
      ...
      cmd[0]=0;
      if(iniValuePtr->scriptsPtr != NULL && (cmdPtr = iniValuePtr->GetIniTableValue(line, iniValuePtr->scriptsPtr)) != NULL)
      { strcpylim(cmd, cmdPtr, 1024);
        ret = CallImportConverterN(DDO, -1, NULL, NULL, NULL, NULL, PDU, Storage, cmd);
      }
      else
      { f = fopen(line, "rt");
        ...
        { if (strstr(line, ".lua"))	// direct call of lua file
          { int i1, n;
            ...
            for (i1=0, n=8; i1<strlen(line); i1++)
            { if (line[i1]=='\\')
              { script[n++] = '\\';
                script[n++] = '\\';
	      }
...
//  char		szTemp[32];
  ...
  iNbConverters = 99;//bcb? Seem like a strange default, still needed?
  IniValue *iniValuePtr = IniValue::GetInstance();
  if (iniValuePtr->sscscpPtr->ImportPtr != NULL && iniValuePtr->sscscpPtr->ImportPtr->ConverterPtr != NULL)
  {
    iNbConverters = iniValuePtr->sscscpPtr->ImportPtr->ConverterPtr->GetSize();
  }
...
{ char *data = (char *)malloc(q->entrysize);
  ...
  { while (1)
	...
	// only try to process if last failure was more than FailHoldOff seconds ago
        ...
        if (time(NULL)-q->lastfailtime > iniValuePtr->sscscpPtr->FailHoldOff)
      ...
      if (time(NULL)-q->lastfailtime>iniValuePtr->sscscpPtr->RetryDelay && q->failfile[0] && NumIndexing==0)
...
{ char data[1536];
  ...
  if (!*q) 
  { IniValue *iniValuePtr = IniValue::GetInstance();
    *q = new_queue(iniValuePtr->sscscpPtr->QueueSize, 1536, 100, exportprocessN, N>=MAXExportConverters ? NULL : ForwardPDU[N], 0);
    sprintf((*q)->failfile, "ExportFailures%s_%d", iniValuePtr->sscscpPtr->Port, N);
    (*q)->ForwardLastUID = (char *) malloc(MAXExportConverters * 66);
    memset((*q)->ForwardLastUID, 0, MAXExportConverters * 66);

    (*q)->pdu_refreshdelay = iniValuePtr->sscscpPtr->ForwardAssociationRefreshDelay;

    (*q)->pdu_closedelay = iniValuePtr->sscscpPtr->ForwardAssociationCloseDelay;

    (*q)->pdu_release = iniValuePtr->sscscpPtr->ForwardAssociationRelease;

    (*q)->maxfails   = iniValuePtr->sscscpPtr->MaximumExportRetries;
  }
...
{ int		i, iNbConverters;
  ...
  IniValue *iniValuePtr = IniValue::GetInstance();
  if (iniValuePtr->sscscpPtr->ExportPtr == NULL
        || iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr == NULL
        || (iNbConverters = iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr->GetSize()) == 0) return;
  if (iNbConverters>MAXExportConverters) iNbConverters=MAXExportConverters;
...
{ char data[2048];
  ...
  if (!q) 
  { IniValue *iniValuePtr = IniValue::GetInstance();
    q = new_queue(iniValuePtr->sscscpPtr->QueueSize, 2048, 100, copyprocess, NULL, 0);
    sprintf(q->failfile, "CopyFailures%s", iniValuePtr->sscscpPtr->Port);
  }
  if (pszFileName==NULL) return;
...
//	char error[256];
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
	sprintf(Tables, "%s", iniValuePtr->sscscpPtr->ImageTableName);
...
/* This code left for reference and speed comparison
		if (MaxRead<0)// bcb? In all uses MaxRead = 0!  UINT < 0 always false!
			{
...
{	PDU_Service		PDU;
	...
	BYTE			SOP[64];
	BYTE			AppTitle[64], host[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compr[ACRNEMA_COMP_LEN];
	int			rc = 0;

	IniValue *iniValuePtr = IniValue::GetInstance();
	if (strcmp(pszSourceAE, "(local)")==0)
  	  pszSourceAE = iniValuePtr->sscscpPtr->MyACRNema;

	PDU.AttachRTC(&VRType);

	PDU.ClearAbstractSyntaxs();
	PDU.SetLocalAddress((unsigned char*)iniValuePtr->sscscpPtr->MyACRNema);
	...
	if (strcmp(pszSourceAE, iniValuePtr->sscscpPtr->MyACRNema)==0)
		{
		if(!PDU.Connect((BYTE *)"127.0.0.1", (BYTE *)(iniValuePtr->sscscpPtr->Port))) 
			{
		...
		{
		if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild(pszSourceAE, (char *)host, (char *)port, (char *)compr))
			{
...
{	PDU_Service		PDU;
	...
	VR			*vr;
	LE_UINT16		command, datasettype, messageid, priority;
	BYTE			SOP[64], AppTitle[64], host[ACRNEMA_IP_LEN],
                    	port[ACRNEMA_PORT_LEN], compr[ACRNEMA_COMP_LEN];
	BOOL			rc = FALSE;
		...
		if(DCOR.GetUINT16(0x0000, 0x0900)==0x0000)
			...
			*StatusString=0;
			...
			if (callback && *callback)
				{
				struct scriptdata sd = {&globalPDU, &DCOR, NULL, -1, NULL, NULL, NULL, NULL, NULL, 0, 0};
				do_lua(&(globalPDU.L), (char *)callback, &sd);
...
{ if (memicmp(data+65, "prefetch",     8)==0) PrefetchPatientData(data, 0);
  IniValue *iniValuePtr = IniValue::GetInstance();
  if (memicmp(data+65, "forward ",     8)==0) return DcmMove  (data, iniValuePtr->sscscpPtr->MyACRNema, data+82, data+100, data+165, data+230, data+256, data+276, data+300, data+366, data+500, 5, data+570);
  if (memicmp(data+65, "preretrieve", 11)==0) return DcmMove  (data, data+82, iniValuePtr->sscscpPtr->MyACRNema, data+100, data+165, data+230, data+256, data+276, data+300, data+366, data+500, 0xfbad, data+570);
  if (memicmp(data+65, "get ",         4)==0) return DcmMove  (data, data+82, iniValuePtr->sscscpPtr->MyACRNema, data+100, data+165, data+230, data+256, data+276, data+300, data+366, data+500, 0xfbad, data+570);
  ...
  { char *p = strchr(data+230, ' ');
    ...
    { struct scriptdata lsd1 = {PDU, NULL, NULL, -1, NULL, NULL, NULL, NULL, NULL, 0, 0};
	...
	{ script[n++] = data[i+230];
      ...
      lua_setvar(&globalPDU, "command_line",    p+1);      
...
{ char data[1000];
  ...
  IniValue *iniValuePtr = IniValue::GetInstance();
  if (delay==0) delay = iniValuePtr->sscscpPtr->ForwardCollectDelay;
  ...
  prefetchPDU.ThreadNum = 0;
    ...
    q = new_queue(iniValuePtr->sscscpPtr->QueueSize, 1000, 100, prefetchprocess, &prefetchPDU, 0);
    q->mayprocess = mayprefetchprocess;
    sprintf(q->failfile, "DelayedFetchForwardFailures%s", iniValuePtr->sscscpPtr->Port);

    q->maxfails = iniValuePtr->sscscpPtr->MaximumDelayedFetchForwardRetries;   //= atoi(szTemp);
  }
...
//		strcat(QueryString, "'");
	...
	if (I==0) return TRUE;					// nothing to do
								...
								// no db access
	if (!DB->Query(IniValue::GetInstance()->sscscpPtr->WorkListTableName, Fields, QueryString, NULL))
		return FALSE;
...
//	char		SOPInstanceUID[256];
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
	if (iniValuePtr->sscscpPtr->WorkListMode==1) 			// optionally update information from worklist db
		ApplyWorklist(DDOPtr, &DB);
	else if (iniValuePtr->sscscpPtr->WorkListMode==2) 		// compulsory update information from worklist db
		{
	...
	*/
	...
	if (pVR && pVR->Data)
		...
		if ((strcmp(s, "1.2.840.10008.1.2")   != 0) &&
			...
			UseChapter10 = has_dcm_extension || iniValuePtr->sscscpPtr->NoDicomCheck;
			}
		else
			{
			UseChapter10 = has_dcm_extension || iniValuePtr->sscscpPtr->NoDicomCheck;
...
//	if ( Edition != E_PERSONAL )
	if ( iniValuePtr->sscscpPtr->Edition != E_PERSONAL )
		{
		if(szSop[0]!='\0')
...
{ OperatorConsole.printf("Started zip and cleanup thread\n");
  ...
  { Sleep(1000);
    ...
    char logfolder[512], timehhmmss[16], *BaseDir;
    struct tm tmbuf;
    time_t t = time(NULL);
    localtime_r(&t, &tmbuf);
    ...
    IniValue *iniValuePtr = IniValue::GetInstance();
    BaseDir = iniValuePtr->GetBasePath();
    if(timeofdayeq( &tmbuf, &(iniValuePtr->sscscpPtr->ziptime)))
    { if (!ZipTriggered)
	...
	if (iniValuePtr->sscscpPtr->ziptime.tm_hour<8) t = time(NULL) - 24*3600;
	else                 t = time(NULL);
        localtime_r(&t, &tmbuf);
	sprintf(logfolder + strlen(logfolder), "%clogs_%04d%02d%02d.zip", PATHSEPCHAR, tmbuf.tm_year+2000, tmbuf.tm_mon+1, tmbuf.tm_mday);// bcb Was tm_year+1900.
    ...
    if (memicmp(timehhmmss, "01:00", strlen("01:00"))==0 && iniValuePtr->sscscpPtr->NightlyCleanThreshhold) //atoi(NightlyCleanThreshhold))
    { if (!CleanTriggered)
      { CleanTriggered = TRUE;
	if (LargestFreeMAG()< iniValuePtr->sscscpPtr->NightlyCleanThreshhold)//(unsigned int)atoi(NightlyCleanThreshhold))
	{ OperatorConsole.printf("Dgate is performing disk cleanup\n");
                    PanicKillOff(iniValuePtr->sscscpPtr->NightlyCleanThreshhold);///(unsigned int)atoi(NightlyCleanThreshhold));
	}
      }
    }
    else
	...
	globalPDU.ThreadNum = 0;
        ...
        if(iniValuePtr->luaPtr != NULL)
          do_lua(&(globalPDU.L), iniValuePtr->luaPtr->nightly, &sd);// was cmd
	if (sd.DDO) delete sd.DDO;
      }
    }
    else
    ...
    { ScriptTriggered = TRUE;
      ...
      struct scriptdata sd = {&globalPDU, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, 0, 0};
      if(iniValuePtr->luaPtr != NULL)
        do_lua(&(globalPDU.L), iniValuePtr->luaPtr->background, &sd);//was cmd
      if (sd.DDO) delete sd.DDO;
      ScriptTriggered = FALSE;
    }
  }
	...
	public:
		...
		//OperatorConsole.printf("C-Store (RTC class)\n");
			...
			if(!rc)
				{
				//delete DDO;
				return iniValuePtr->sscscpPtr->StorageFailedErrorCode;	// Processing failed
				}
...
{ BOOL cache = FALSE, cacheseries = TRUE, nonvirtual=FALSE;
  ...
  IniValue *iniValuePtr = IniValue::GetInstance();
  if(iniValuePtr == NULL || iniValuePtr->sscscpPtr->VirtualServerForPtr == NULL ||
     N >= (int)iniValuePtr->sscscpPtr->VirtualServerForPtr->GetSize()) return 0;
  char *vsfn = iniValuePtr->sscscpPtr->VirtualServerForPtr->Get(N);
//  if (*VirtualServerFor[N]==0) return 0;
  if (strstr(vsfn, ",CACHE"))        cache       = TRUE;
  if (strstr(vsfn, ",CACHESTUDIES")) cacheseries = FALSE;
  if (strstr(vsfn, ",NONVIRTUAL"))   nonvirtual  = TRUE;
...
int VirtualQuery(DICOMDataObject *DDO, const char *Level, int N, Array < DICOMDataObject  *> *pADDO, char *ae)
	...
	*AE = NULL;
	IniValue *iniValuePtr = IniValue::GetInstance();
	if(iniValuePtr->sscscpPtr->VirtualServerForPtr != NULL &&
        N >= (int)iniValuePtr->sscscpPtr->VirtualServerForPtr->GetSize())
        strcpylim(AE, iniValuePtr->sscscpPtr->VirtualServerForPtr->Get(N), 48);
	if (ae) strcpylim(AE, ae, 48);
	if (*AE==0) return FALSE;

        char *param = strchr(AE, ',');
        if (param) *param++ = 0;

	unsigned char 	ip[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN], SOP[66];
	VR		*vr;
	UID		uid;
	...
	if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild(AE, (char *)ip, (char *)port, (char *)compress))
		return 0;
	...
	if (param)
		if (strstr(param, "FIXKODAK") || strstr(param, "fixkodak"))  // disallow wildcard date queries
			...
			if (!(iniValuePtr->sscscpPtr->FixKodak))
				{
	...
	PDU.ClearAbstractSyntaxs();
	PDU.SetLocalAddress((BYTE *)iniValuePtr->sscscpPtr->MyACRNema);
	...
	PDU.SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);
...
int VirtualQueryToDB(DICOMDataObject *DDO, int N, char *ae)
	{
	char AE[48];
	*AE = 0;
	IniValue *iniValuePtr = IniValue::GetInstance();
	if(iniValuePtr->sscscpPtr->VirtualServerForPtr != NULL &&
            N >= (int)iniValuePtr->sscscpPtr->VirtualServerForPtr->GetSize())
            strcpy(AE, iniValuePtr->sscscpPtr->VirtualServerForPtr->Get(N));
	if (ae) strcpy(AE, ae);
	if (*AE==0) return FALSE;
	...
	for (int i=0; i<BDDO.GetSize(); i++)
		...
		*rRoot=1;
		*rFilename=0;
		if (!GenerateFileName(DDOR2, Device, rRoot, rFilename, TRUE, 0, iniValuePtr->sscscpPtr->MyACRNema, (char *)AE, &DB))
			OperatorConsole.printf("**** Virtual query (%s) filename problem - %s\n", AE, rFilename);
	        if (memcmp(rFilename, "lua:", 4)==0) lua_getstring(&globalPDU, NULL, DDOR2, rFilename+4, rFilename);
...
BOOL	PatientStudyFinder(char *server, char *str, char *fmt, FILE *f, const char *level)
	...
	char outp[7][256];
//	BOOL local=stricmp(server, IniValue::GetInstance()->sscscpPtr->MyACRNema)==0;
	Array < DICOMDataObject * > ADDO;
	unsigned int i, j;
	tmp2[0]=0; tmp[0]=0;
...
static int ProcessRATData(struct ReadAheadThreadData *ratd, int maxfiles)
	...
	for (i=0; i<ratd->NumToRead; i++)
		...
		if (len>4)
			UseChapter10 = (stricmp(FullFilename+len-4, ".v2")!=0) || iniValuePtr->sscscpPtr->NoDicomCheck;
	...
	*AE = 0;
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
            if( iniValuePtr->sscscpPtr->VirtualServerForPtr != NULL &&
                   N >= (int)iniValuePtr->sscscpPtr->VirtualServerForPtr->GetSize())
                        strcpylim(AE, iniValuePtr->sscscpPtr->VirtualServerForPtr->Get(N),48);
            if( iniValuePtr->sscscpPtr->VirtualServerPerSeriesPtr != NULL &&
                   N >= (int)iniValuePtr->sscscpPtr->VirtualServerPerSeriesPtr->GetSize())
                        PerSeries = *(iniValuePtr->sscscpPtr->VirtualServerPerSeriesPtr->Get(N));
	if (*AE==0) return FALSE;

        char *param = strchr(AE, ',');
        if (param) *param++ = 0;

  	unsigned char 		ip[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN];
	VR			*vr;
	char			SOP[66];
	...
	char			*sops, *p;
	int			*seriesnum;
	char			pats[66], series[66], studies[66];

	ratd->overlap = iniValuePtr->sscscpPtr->OverlapVirtualGet;
	...
	PDU2.SetLocalAddress((BYTE *)iniValuePtr->sscscpPtr->MyACRNema);
	...
	PDU2.AddAbstractSyntax(uid);
        ...
        PDU2.SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);
	...
	for (i=0; i<=ratd->NumToRead; i++)
		...
		// image i is from a new series, send series belonging to image i-1
			...
			vr = new VR (0x0000, 0x0110, 0x0002, &messageid, FALSE);
			DCO.Push(vr);
			vr = new VR(0x0000, 0x0600, 16, (void*)iniValuePtr->sscscpPtr->MyACRNema, FALSE);
			DCO.Push(vr);
...
static BOOL WINAPI ReadAheadThread(struct ReadAheadThreadData *ratd)
	...
	if (ratd->filesmissing>0 && !ratd->ForceEnd)
		...
		for (int i1=0; i1<10; i1++)
			if (VirtualServer2(ratd, i1)) break;
		}

	if (ratd->filesmissing>0)
		{
		OperatorConsole.printf("*** Virtual server failed to retrieve some files:\n");
				...
				strcpy(Filename, ratd->ObjectFiles + i1 * 256);
			  	...
			  	if (!FindPhysicalDevice(Device, ratd->FullFilenames+i1*1024, Filename))
					...
					OperatorConsole.printf("*** missing file: %s %s\n", Device, Filename);
                			DeleteImage(ratd->SOPs+i1*66, TRUE);
			  		}
				}
		}
...
*/
		...
		if (!iniValuePtr->sscscpPtr->CacheVirtualData)
			{
...
RetrieveOn (
	...
	*DDOOutPtr = NULL;
	...
	if (!iniValuePtr->sscscpPtr->EnableReadAheadThread || NoThread) pADDO = NULL;
	...
	if (pADDO && pADDO->Get(0)==DDOMask && pADDO->GetSize() > 0)
		...
		ratd->numreadahead = iniValuePtr->sscscpPtr->EnableReadAheadThread<2 ? 5 : iniValuePtr->sscscpPtr->EnableReadAheadThread;
		ratd->overlap = 0;
	...
	if (pADDO && ratd->Handle!=NULLHANDLE && !ratd->TimeOut)
		...
		while (!ratd->TimeOut)
			...
			for (i=0; i<ratd->NumToRead; i++)
				{
				if (!ratd->reverse && pADDO->Get(i)==DDOMask && (ratd->Status[i]==STATUS_READ || ratd->Status[i]==STATUS_FAILED)) 
					{
				...
				if (pADDO->Get(i)==DDOMask && (ratd->Status[ratd->NumToRead-1-i]==STATUS_READ || ratd->Status[ratd->NumToRead-1-i]==STATUS_FAILED)) 
					{
					ratd->reverse = TRUE;
...
further:
	...
	//	RTCStorage.DB.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
		//	iniValuePtr->sscscpPtr->Password, iniValuePtr->sscscpPtr->DataHost );
...
QualifyOn(
	...
	{
	IniValue *iniValuePtr = IniValue::GetInstance();
	strcpy((char*)MyACRNema, iniValuePtr->sscscpPtr->MyACRNema);
	if(!ACRNemaMap::GetInstance()->GetACRNema((char*)ACRNema, (char*)RemoteIP, (char*)RemotePort, (char*)Compress))
		{
	...
	*/
	...
	if(iniValuePtr->sscscpPtr->SendUpperCaseAE) strupr((char*)ACRNema);
...
BOOL	DriverApp	::	Server ( BYTE	*port )
...
#endif
	MSocket.SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);
	...
	while (MSocket.Accept() )
        ...
        //Save the remote IP address and get it quick.
//		strcpylim(ConnectedIPStr, MSocket.ConnectedIPAddress(), 16);
		ConnectedIP = MSocket.sa.sin_addr.s_addr;    
		while ( Lock )
			Sleep(50);
		Lock = 1;

//		ConnectedIP = MSocket.sa.sin_addr.s_addr;//bcb? I had trouble when I grabed it here.
...
//			if(ACRNemaMapPtr) delete ACRNemaMapPtr;
			free(PatientDB);
			free(StudyDB);
			free(SeriesDB);
			free(ImageDB);
...
BOOL	DriverApp	::	ServerChildThread ( int	Socketfd, unsigned int remoteIP )
	{
	//ExitThread ( (int) ServerChild ( Socketfd ) );
	ServerChild( Socketfd, remoteIP );
	return ( FALSE );
	}
...
#endif
	int	Socketfd;
//	char	remoteIP[DOT_FOUR_LEN];
	unsigned int remoteIP;

	// pick up data from passed-in object

	Socketfd = App->ChildSocketfd;
//	strcpylim(remoteIP, App->ConnectedIPStr, DOT_FOUR_LEN);//Save it again (App disapears)
	remoteIP = App->ConnectedIP;
	// release lock on data

	App->Lock = 0;

	App->ServerChildThread ( Socketfd, remoteIP );
...
{ char host[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN];
  char *p;
  RunTimeClassStorage	RTCStorage;
  ...
  if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild(server, (char *)host, (char *)port, (char *)compress))
  { strcpy(host, server);
    p = strchr(host, ':');
    if (p) 
    { *p=0;
...
BOOL StorageApp ::	StorageCommitmentSupport( CheckedPDU_Service *PDU, DICOMCommandObject *DCO, DICOMDataObject **CommitData)
	...
	BYTE ip[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compress[ACRNEMA_COMP_LEN];
	int nfail=0;
	BOOL status;
	...
	if (*CommitData)
		...
		PDU2.AttachRTC(&VRType);
		if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild(Calling, (char *)ip, (char *)port, (char *)compress))
			{
	...
	*CommitData = new DICOMDataObject;
	if (ActionStorageCommitmentPushRequest.Read(DCO, PDU, *CommitData))
		...
		IniValue *iniValuePtr = IniValue::GetInstance();
		SetStringVR(&vr, 0x0008, 0x0054, iniValuePtr->sscscpPtr->MyACRNema); 
		(*CommitData)->Push(vr);
...
void NewTempFile(char *name, const char *ext)
	...
	if (iniValuePtr->sscscpPtr->TempDir[0]==0)
		...
		strcpy(name, iniValuePtr->sscscpPtr->TempDir);
...
BOOL StorageApp	::	ServerChild (int theArg, unsigned int remoteIP )
	{
	IniValue *iniValuePtr = IniValue::GetInstance();
	ACRNemaMap *ACRNemaMapPtr = ACRNemaMap::GetInstance();
	ExtendedPDU_Service	PDU ( iniValuePtr->sscscpPtr->SOPClassFile );
	DICOMCommandObject	DCO;
	DICOMDataObject		*DDO;
	VR			*vr;
//	VR			*vr1;
	...
	ThreadNum = 0;
#endif
	DebugLevel= iniValuePtr->sscscpPtr->DebugVRs;
	TimeOfDay = time(NULL);
	...
	ratd.RemoteIP = remoteIP;//Save the remote IP address

	if (!DB1.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
		iniValuePtr->sscscpPtr->Password, iniValuePtr->sscscpPtr->DataHost ) )
		{
	...
	OpenThreadCount++;
        ...
        struct scriptdata lsd1 = {&PDU, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, 0, ConnectedIP};
	if(iniValuePtr->luaPtr != NULL) do_lua(&(PDU.L), iniValuePtr->luaPtr->association, &lsd1);
	else do_lua(&(PDU.L), "", &lsd1);//bcb? needed?
	if (lsd1.DDO) delete lsd1.DDO;
	if (lsd1.rc==2 || lsd1.rc==6) return TRUE; // reject association
	...
	while(TRUE)
		...
		struct scriptdata sd2 = {&PDU, &DCO, NULL, 0, NULL, NULL, NULL, NULL, NULL, 0, ConnectedIP};
	        if(iniValuePtr->luaPtr != NULL) do_lua(&(PDU.L), iniValuePtr->luaPtr->command, &sd2);
	        else do_lua(&(PDU.L), "", &sd2);//bcb? needed?
		...
		if (orgmessageid==0xfbad)
			{
			sprintf(OwnAE, "%-16s", iniValuePtr->sscscpPtr->MyACRNema);
		...
		if (SOPVerification.ReadRequest(&PDU, &DCO))
			...
			if (SilentText[0])
				...
				if (memcmp(SilentText, "lua:", 4)==0)
					...
					do_lua(&(globalPDU.L), SilentText+4, &sd);
					if (sd.DDO) delete sd.DDO;
					}
				...
				if (memcmp(SilentText, "luastart:", 9)==0)
					...
					struct scriptdata sd = {&globalPDU, &DCO, NULL, -1, NULL, NULL, NULL, NULL, NULL, 0, ConnectedIP};
					PDU.Close();
					do_lua(&(globalPDU.L), SilentText+9, &sd);
					if (sd.DDO) delete sd.DDO;
					}
					...
					if (DB.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
						...
						if (DB.db_type == DT_DBASEIII && SilentText[8])
							{
					 		sprintf(t, "%sX%s.DBF", iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->PatientTableName);
							DB.ExtractRecords("DICOMPatients",SilentText+8, t);
					 		sprintf(t, "%sX%s.DBF", iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->StudyTableName);
							DB.ExtractRecords("DICOMStudies", SilentText+8, t);
					 		sprintf(t, "%sX%s.DBF", iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->SeriesTableName); 
							DB.ExtractRecords("DICOMSeries",  SilentText+8, t);
					 		sprintf(t, "%sX%s.DBF", iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->ImageTableName);  
							DB.ExtractRecords("DICOMImages",  SilentText+8, t);
					 		//sprintf(t, "%sX%s.DBF", DataSource, iniValuePtr->sscscpPtr->WorkListTableName);
							...
							for (int j=0; j<5; j++)
								...
								switch(j)
									{
									case 0: dbe = PatientDB; tabname = iniValuePtr->sscscpPtr->PatientTableName; break;
									case 1: dbe = PatientDB; tabname = iniValuePtr->sscscpPtr->PatientTableName; break;
									case 2: dbe = StudyDB;   tabname = iniValuePtr->sscscpPtr->StudyTableName;   break;
									case 3: dbe = SeriesDB;  tabname = iniValuePtr->sscscpPtr->SeriesTableName;  break;
									case 4: dbe = ImageDB;   tabname = iniValuePtr->sscscpPtr->ImageTableName;   break;
									}
					...
					DB.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
					      iniValuePtr->sscscpPtr->Password, iniValuePtr->sscscpPtr->DataHost);
					}
					...
					*p++=0;				// points after 1st comma
					...
					iniValuePtr->sscscpPtr->DebugVRs = DebugLevel = atoi(SilentText+11);
					...
					if (SilentText[12]>='0' && SilentText[12]<='9')
						...
						SystemDebug.Off();
						OperatorConsole.Off();
						OperatorConsole.OnUDP(iniValuePtr->sscscpPtr->OCPipeName, SilentText+12);
						SystemDebug.OnUDP(iniValuePtr->sscscpPtr->OCPipeName, SilentText+12);
					...
					if (SilentText[7]>='0' && SilentText[7]<='9')
						...
						SystemDebug.Off();
						OperatorConsole.Off();
						OperatorConsole.OnUDP(iniValuePtr->sscscpPtr->OCPipeName, SilentText+7);
						OperatorConsole.AddTimeStamps(0);
						StopZipThread();
						}
				...
				else if (memcmp(SilentText, "read_amap", 9)==0)//: optional (not needed)
					{
//					CloseACRNemaAddressArray();
//					if(!InitACRNemaAddressArray())
//					char *fullPath = getAmapFileName();
					if(iniValuePtr != NULL && iniValuePtr->sscscpPtr->ACRNemaMap[0] != 0)
						{
						if(!(ACRNemaMapPtr->InitACRNemaAddressArray(iniValuePtr->sscscpPtr->ACRNemaMap)))// Will close if open.
							{
...
//						free(fullPath);
					...
					{
					if(!iniValuePtr->ReadIni()) OperatorConsole.printf("***Error read ini file:%s\n", iniValuePtr->errorBufferPtr);
//					ConfigDgate();
//					ConfigMicroPACS();
					}
					...
					p = SilentText+8;
					if (*p==0) p = ps;
					f = fopen(iniValuePtr->GetIniFile(),     "rt");
					NewTempFile(tempfile, ".txt");
					g = fopen(tempfile, "wt");
					...
					if (p) *p++=0;				// points after 1st comma
					if (p==NULL) p = ps;
					Parameter = iniValuePtr->GetIniString(SilentText+10);
					if(Parameter != NULL)
						{
						snprintf(Response, 512, p, Parameter);
						free(Parameter);
						}
					...
					if (p) *p++=0;				// points after 1st comma
					if (p==NULL) p = ps;
					Parameter = iniValuePtr->GetIniString(SilentText+14);
					if(Parameter != NULL)
						{
						snprintf(Response, 512, p, Parameter);
						free(Parameter);
...
/*					int r, i;
					...
					char line[512];*/
 					if (p) *p++=0;				// points after 1st comma
 					if (p==NULL) p = ps;
 					Parameter = iniValuePtr->GetIniLine(SilentText+12);
 					if(Parameter != NULL)
						{
						snprintf(Response, 512, p, Parameter);
						free(Parameter);
...
/*					FILE *f, *g;
					...
					if (p) *p++=0;				// points after 1st comma
					strcpy(newConfigFile, iniValuePtr->GetIniFile());
					...
					fclose(f);
					fclose(g);
					unlink(iniValuePtr->GetIniFile());
					rename(newConfigFile, iniValuePtr->GetIniFile());*/
					if (p) *p++=0;
					if(iniValuePtr->WriteToIni(SilentText+10, p))
						if(!iniValuePtr->ReadIni()) OperatorConsole.printf("***Error reading from ini file:%s\n", iniValuePtr->errorBufferPtr);
					else OperatorConsole.printf("***Error writing to ini file:%s\n", iniValuePtr->errorBufferPtr);
...
/*					FILE *f, *g;
					...
					unlink(iniValuePtr->GetIniFile());
					rename(newConfigFile, iniValuePtr->GetIniFile());*/
					if(!iniValuePtr->DeleteNameFromIniFile(SilentText+13))
						OperatorConsole.printf("***Error deleting ini parm: %s\n", iniValuePtr->errorBufferPtr);
					if(!iniValuePtr->ReadIni())
						OperatorConsole.printf("***Error read ini file: %s\n", iniValuePtr->errorBufferPtr);
//					FlushPrivateProfileStringCache();
					}
...
//					char pq[] = "%-17s %-30s %-10s %-16s";
					...
					ACRNemaMapPtr->GetOneEntry( r, Response, 512, p);
/*					if (p==NULL) p = pq;
				...
				else if (memcmp(SilentText, "get_amaps", 9)==0)// Optional :, except for format
					{
//					char pq[] = "%-17s %-30s %-10s %-16s\n";
//					FILE *g;
//					p = SilentText+10;
//					if (*p==0) p = pq;
					NewTempFile(tempfile, ".txt");
//					g = fopen(tempfile, "wt");
					ACRNemaMapPtr->WriteAMapToAFile( tempfile, SilentText+10 );
/*					for (unsigned int r=0; r< ACRNemaAddressArray.GetSize(); r++ )
				...
				else if (memcmp(SilentText, "write_amap", 10)==0)// Optional :, except for format
					{
//					char pq[]="%-17s %-30s %-10s %-16s\n";
//					unsigned int r = 0;
					if (p) *p++=0;				// points after 1st comma
					ACRNemaMapPtr->WriteAMapToTheFile(SilentText + 11);
					}

				else if (memcmp(SilentText, "put_amap:", 9)==0)
					{
					unsigned int cnt, replaceLine;
					char *silentTextEnd;
					char *ae, *items[3];
                                                                    
					if (p)// if not first ",", nothing to do!
						{
						silentTextEnd = SilentText + strnlen( SilentText, 64000 );
						*p++=0;				// points after 1st comma
						replaceLine = atoi(SilentText+9);
						ae = p;//First "," followed by AETitle
						items[1] = items[2] = NULL;// For test and default later
						for (cnt = 0; cnt < 3; cnt++)
							{
							while(*p != ',')
                                                                                {
                                                                                    p++;
                                                                                    if(p >= silentTextEnd) break;//while
                                                                                }
							if(*p != ',') break;//Overrun escape for default compression.
							*p++=0;//Treminate and move on
							items[cnt] = p;
							}
						if(items[1] != NULL) // We need the port!
							{
							ACRNemaMapPtr->ReplaceEntry(replaceLine, ae, items[0], items[1], items[2]);
							}
						}
					}
/*					int i, n=1, L;
					...
					if (items[4]) strcpy(AAPtr->Compress, items[4]);				
						...
						ACRNemaMapPtr->DeleteEntry(atoi(SilentText+12));
/*					{
					ACRNemaAddress	*AAPtr;
					...
					FILE *f = fopen(iniValuePtr->sscscpPtr->SOPClassFile, "wt");
					fprintf(f, "#\n");
					...
					if (pDDO) 
						...
						if (q==NULL || *q==0)
							...
							ToJPG(pDDO, tempfile, atoi(p), 0, level, window, frame);
							}
						else if (strcmp(q, "cgi")==0)
							{
							NewTempFile(tempfile, ".jpg");
							...
							fclose(f);
							ToJPG(pDDO, tempfile, atoi(p), 1, level, window, frame);
							}
						else
							ToJPG(pDDO, q, atoi(p), 0, level, window, frame);
						ImagesToGifFromGui++;
                                        	delete pDDO;
						}
					}
...
//						char blank[]="";// Shadow, not needed
					...
					{
					if (p) *p++=0;				// points after 1st comma
					GrabImagesFromServer((unsigned char *)SilentText+21, p, iniValuePtr->sscscpPtr->MyACRNema);
					GrabFromGui++;
					}
...
//			}
		...
		if(iniValuePtr->luaPtr != NULL) do_lua(&(PDU.L), iniValuePtr->luaPtr->clienterror, &sd3);
		else do_lua(&(PDU.L), "", &sd3);//bcb? needed?
		...
		if(iniValuePtr->luaPtr != NULL) do_lua(&(PDU.L), iniValuePtr->luaPtr->endassociation, &sd4);
		else do_lua(&(PDU.L), "", &sd4);//bcb? Needed?
...
//	char	szTemp[32];
	int	i;
	BOOL	MissingDict=FALSE, noW = TRUE;
//	char	tempStr[256], *baseDir, *configFile;
	int	valid_argc;
//	struct	stat st;
	...
	{
		if(!(iniValuePtr->SetBasePath(argv[0])))// Set the base path to the current dgate path.
		{
			fprintf(stderr, "%s", iniValuePtr->errorBufferPtr);
			exit(EXIT_FAILURE);
		}
	}
	else
	{
#ifdef UNIX // Set the base path to a default.
		if(!(iniValuePtr->SetBasePath("./")))
#else
		if(!(iniValuePtr->SetBasePath(".\\")))
#endif
		{
			fprintf(stderr, "Can not set the directory to the default");
			exit(EXIT_FAILURE);
		}
	}
	for (valid_argc = 1; valid_argc < argc; valid_argc++ )
		...
		if((argv[valid_argc][0]=='-') || (argv[valid_argc][0]=='/'))
			...
			if( argv[valid_argc][1] == 'w' || argv[valid_argc][1] == 'W')
				{
				if(!(iniValuePtr->SetIniFile(argv[valid_argc] + 2)))// Drop the -w.
					{
					fprintf(stderr, "%s", iniValuePtr->errorBufferPtr);
					exit(EXIT_FAILURE);
					}
	...
	// no -w: get ini file from same directory as binary
	if (noW && !(iniValuePtr->SetIniFile(NULL)))
			{
			fprintf(stderr, "%s", iniValuePtr->errorBufferPtr);
			exit(EXIT_FAILURE);
			}
	// Does anything work without it?  Read the Ini file.
	if (!iniValuePtr->ReadIni())
	{
			if(iniValuePtr->IniDumped())
				{
				fprintf(stderr, "%s\n",iniValuePtr->errorBufferPtr);
				exit(EXIT_SUCCESS);
				}
			fprintf(stderr, "Error reading Ini file: %s\n %s\n",iniValuePtr->GetIniFile(), iniValuePtr->errorBufferPtr);
			exit(EXIT_FAILURE);
	}

	if(iniValuePtr->sscscpPtr->MicroPACS == NULL)
		{
	...
	if (!VRType.AttachRTC(iniValuePtr->sscscpPtr->Dictionary))
		{
		MissingDict = TRUE;
		printf("Dictionary file %s is missing, some server functions will not work correctly\n",
                               iniValuePtr->sscscpPtr->Dictionary);
		}

	if (!(iniValuePtr->sscscpPtr->NoDicomCheck))
		SetDicomErrorHandler(MyDcmError);
	...
	globalPDU.ThreadNum = 0;
        struct scriptdata sd = {&globalPDU, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, 0, 0};
	if(iniValuePtr->luaPtr != NULL) do_lua(&(globalPDU.L), iniValuePtr->luaPtr->startup, &sd);
	if (sd.DDO) delete sd.DDO;
	if (sd.rc==2 || sd.rc==6) exit(EXIT_SUCCESS); // reject on startup
...
//	_CrtSetBreakAlloc(37);
	...
	if ( ! (Socketfd = ParseArgs(argc, argv, &globalPDU) ))
		...
		if (RunServer)
			...
			{
			Database testDB;
			if (testDB.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
				iniValuePtr->sscscpPtr->Password, iniValuePtr->sscscpPtr->DataHost ) )
				{ 
			...
			iNbConverters = 0;
			if (iniValuePtr->sscscpPtr->ExportPtr != NULL && iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr != NULL)
			    iNbConverters = iniValuePtr->sscscpPtr->ExportPtr->ConverterPtr->GetSize();
			if (iNbConverters>MAXExportConverters) iNbConverters=MAXExportConverters;
...
/* This did not work.  MIRROR0 in not a dicom.ini name, MIRRORDevices would be the correct name.  bcb?
			...
			if(iniValuePtr->sscscpPtr->MIRRORDevicePtr != NULL &&
			    iniValuePtr->sscscpPtr->MIRRORDevicePtr->GetSize() != 0 && !NoThread)
			{ mirrorcopy_queue(NULL, NULL);
			  OperatorConsole.printf("Started mirror copy queue thread\n");
			}

			// Start prefetcher queue 
			if (iniValuePtr->sscscpPtr->Prefetcher && !NoThread)
			{ prefetchermode = iniValuePtr->sscscpPtr->Prefetcher;//atoi(szTemp);
			  prefetcher(NULL, FALSE);
			  OperatorConsole.printf("Started prefetcher queue thread\n");
			}

			if (iniValuePtr->sscscpPtr->monitorfolder2[0] != 0)
            {
			  OperatorConsole.printf("Also monitoring for files in: %s\n", iniValuePtr->sscscpPtr->monitorfolder2);
			  StartMonitorThread(iniValuePtr->sscscpPtr->monitorfolder2);
			}

			if (NoThread)
				while ( TRUE )
					ServerApp.Server((BYTE *)(iniValuePtr->sscscpPtr->Port));
			else
				ServerApp.Server((BYTE *)(iniValuePtr->sscscpPtr->Port));
			}
		return(0);
...
BOOL GrabImagesFromServer(BYTE *calledae, const char *studydate, char *destination)
	...
	if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild((char *)calledae, (char *)ip, (char *)port, (char *)compress))
		return FALSE;
...
BOOL SendServerCommand(const char *NKIcommand1, const char *NKIcommand2, int con, char *buf, BOOL html, BOOL upload)
	{
	PDU_Service		PDU;
	DICOMCommandObject	DCO;
	DICOMCommandObject	DCOR;
	...
	PDU.ClearAbstractSyntaxs();
	PDU.SetLocalAddress((BYTE *)iniValuePtr->sscscpPtr->MyACRNema);
	PDU.SetRemoteAddress((BYTE *)iniValuePtr->sscscpPtr->MyACRNema);
	uid.Set("1.2.840.10008.3.1.1.1");	// Dicom APP
	PDU.SetApplicationContext(uid);
	uid.Set("1.2.840.10008.1.1");		// Verification
	PDU.AddAbstractSyntax(uid);

	if(!PDU.Connect((unsigned char *)ServerCommandAddress,(BYTE *)(iniValuePtr->sscscpPtr->Port)))
		return ( FALSE );
...
static BOOL DcmEcho(const char *AE)
	...
	VR			*vr;
	LE_UINT16		command, datasettype, messageid;//, tuint16;
	BYTE			SOP[64], host[ACRNEMA_IP_LEN], port[ACRNEMA_PORT_LEN], compr[ACRNEMA_COMP_LEN];
	...
	if(!ACRNemaMap::GetInstance()->GetACRNemaNoWild((char *)AE, (char *)host, (char *)port, (char *)compr)) 
		return FALSE;
