...
20120627    bcb Restored bitsstored, highbit and numberofbits for #ifdef FIX_GRYOSCAN.
...
20120703    bcb	Fixed double && in compress JPEGL and brackets in reset "Planes" VR. Fix always false UINT < 0. WIN32 to !
                UNIX removed always false, replaced WIN32 with !UNIX, added jpeg_type, rewrote recompress,
20120710    bcb	Split up dgate into dgate, dgatecgi, dgatehl7 and nkiqrsop into nkiqrsop, jpegconv; moved parseArgs to parse.
                Added mkdir define.  Cleaned up includes.
20120723	mvh	Fixed 'l' or 'l' to 'l' or 'L' found by bcb
20120820    bcb	Added wild card IPs and ACRNemaMap class.
...
*/
unsigned int ComputeCRC(char* pcData, size_t iNbChars)
{ size_t		i;
...
{ static int		res=-1, dn16=1;
  ...
  { IniValue *iniValuePtr = IniValue::GetInstance();
    res = iniValuePtr->sscscpPtr->UseBuiltInJPEG;

    dn16 = iniValuePtr->sscscpPtr->DecompressNon16BitsJpeg;
  }
...
BOOL	StandardRetrieveNKI	::	Read (
	...
	UINT		Index;
	BYTE		IP [ ACRNEMA_IP_LEN ], lPort [ ACRNEMA_PORT_LEN ],
		ACRNema [ ACRNEMA_AE_LEN ], MyACR[ACRNEMA_AE_LEN], Compress[ACRNEMA_COMP_LEN];
...
//	int		ContextID;
	...
	NewPDU.SetTimeOut(iniValuePtr->sscscpPtr->TCPIPTimeOut);
...
//ExtraBytes is ratd struct pointer, who's first entry is remote IP
	strcpylim((char *)IP, inet_ntoa(*(struct in_addr*)ExtraBytes), ACRNEMA_IP_LEN);
	...
	if(((char *)IP)[0] == 0)//Check for wild card ip fail.
		{
		CMoveRSP :: Write (PDU, DCO, 0xc003, 0, 0, 0, 0, NULL );
		SystemDebug.printf("Retrieve: move destination ip may not match\nAE: %s ", ACRNema);
		return ( TRUE );
		}
	...
	if (!NewPDU.Connect (IP, lPort))
		...
		OperatorConsole.printf("Host '%s' did not accept the connection on %s\n", ACRNema, IP);
...
{ unsigned int		iCRC;
  ...
  { iCRC  = 0;
    ...
    *(short int *)dest = val = *src; 
    ...
    pHeader->iCompressedSize = (dest - (signed char*)pHeader - sizeof(NKI_MODE2)) & INT_MAX;
  ...
  return (dest - (signed char*)pHeader) & INT_MAX;
}
...
{// int		rc;
  ...
  if (iNbFrames<=1 || Frame>=iNbFrames /*|| Frame<0*/) return FALSE;// bcb always false UINT < 0
...
{ //int		rc;
  ...
  iNbFrames = pDDO->Getatoi(0x0028, 0x0008);
  if (iNbFrames<=1 || FirstFrame>=iNbFrames/* || FirstFrame<0*/) return FALSE;// bcb always false UINT < 0
  if (LastFrame>=iNbFrames/* || LastFrame<0*/) LastFrame = iNbFrames-1;// bcb always false UINT < 0
  if (skip<1) skip = 1;
...
void SaveDICOMDataObject(char *Filename, DICOMDataObject* pDDO)
	{
	size_t len;
	int has_dcm_extension=0, UseChapter10=FALSE, bForcedImplicit=FALSE;
	VR *pVR;
	char s[256];
	PDU_Service PDU;
...
// If debug > 3, print info.
	if (DebugLevel > 3)
	{//Errors combine to form a better message
...
// Forward declaration of RLE decompressor /*bcb 20120703 moved too hpp
/*static int DecompressRLE(SLICE_INFO* pSliceInfo, VR* pSequence,
	void** ppResult, unsigned int * piResultSize);
...
// end distinguish RLE converted from jpeg without TrnSyntaxUID mvh 20110413
		...
		else if(pVR && pVR->Length == 22 && ((char*)pVR->Data)[20] == '9')//90 or 91 is jpeg 2000.
		{
			...
			if(IniValue::GetInstance()->sscscpPtr->UseOpenJPG)
			{
				if(!DecompressJPEG2Ko(*pDDO))
				{
...
char jpeg_type(VR *vr)
{
    ...
    }// No 2,10 with jpeg start.
    return '0';// What is this, unknown jpeg.
}
...
{
	int 	rc = FALSE, status = 0, compL2, size, qual;
    signed char *p;
    BOOL	pass = false, known = TRUE ;// all;
    VR  *vr;
    ...
    switch (Compression[0])
    {
        ...
        case 'V':
            return TRUE;
        case 'j': //ja,j1,j2,j5,j4,j5,j6,jl,jk
        case 'J':
            vr = (*pDDO)->GetVR(0x0002, 0x0010);
            if (vr && vr->Data)
            {
                char j_type = jpeg_type(vr);
                if ( j_type != '0')
                    if ( Compression[1] == 'a' || Compression[1] == 'A' || Compression[1] == j_type)
                    {
                        if (StripGroup2) Strip2(*pDDO);
                        return TRUE;
                    }
            }
            if (!DecompressImage(pDDO, &status))
            {
                SystemDebug.printf(
                    "[recompress]: decompressed error with mode = %s (strip=%d)\n", Compression, StripGroup2);
                return FALSE;
            }
            qual = 100; // default as sb configured in dicom.ini, 0 may switch to min value now 1
            if (Compression[2]) qual = atoi(Compression+2);
            if (qual < 0 || qual > 100) qual = 100;
            if((rc = CompressJPEGImage(pDDO, Compression[1], &status, qual)))
                if (strchr("3456lL", Compression[1]))
                    (*pDDO)->ChangeVR(0x0028, 0x2110, "01", 'CS'); // 20120624: set lossy compression
            break;
        case 'n': //nj,n0,n1,n2,n3,n4
        case 'N':
            compL2 = Compression[1]-'0';
            if (Compression[1] == 'j' || Compression[1] == 'J')
            {
                compL2 = MAXNKICOMPRESSION;
                vr = (*pDDO)->GetVR(0x0002, 0x0010);
                if (vr && vr->Data && jpeg_type(vr) != '0') pass = TRUE;
            }
            if (!pass) // Will also test nj for MAXNKICOMPRESSION
            {
                vr = (*pDDO)->GetVR(0x7fdf, 0x0010);
                if (vr)
                {
                    p = ((signed char *)(vr->Data));
                    if (p && get_nki_private_compress_mode(p)==compL2) pass = TRUE;
                }
            }
            if ( pass)
            {
            ...
            if (compL2 == 0) rc = TRUE;// Uncompressed NKI
            else                     
			{
                rc = CompressNKI(*pDDO, compL2);
                status = rc;
			}
            break;
        case 'k':
        case 'K':
            ...
            if (TestDownsize(*pDDO, NULL, size))
            {
                ...
                if ( size != 65536 )rc = MaybeDownsize(*pDDO, NULL, size);
                if (rc==2) status = 1;
            }
            else status = 1;
        case 's':
        ...
        case 'i':
        case 'I':
            if ( Compression[1] == 's' || Compression[1] == 'S' ) return TRUE;
        default:
            SystemDebug.printf("[recompress]: Unknown compression %s, using UN", Compression);
        case 'u': //uj,ui,ub,ue,un uncompressed
        case 'U':
            pass = FALSE;
            known = TRUE;
            vr = (*pDDO)->GetVR(0x0028, 0x0004);		 // depalette should happen 20101220
            if(!vr || vr->Length < 14 || !strncmp( (char *)vr->Data, "PALETTE COLOR", 13))
            {//No palettes in Ux
                ...
                {//No planes in Ux
                    ...
                    if (vr && vr->Data && vr->Length > 17 && memcmp(vr->Data, "1.2.840.10008.1.2", 17) == 0)//20130520	bcb
                    {
                        switch (Compression[1])
                        {
                            case 'j': //uj
                            case 'J':
                                if( jpeg_type(vr) != '0' ) pass = TRUE;
                                break;
                            case 'b': //ui
                            case 'B':
                                if (vr->Length > 19 && ((char *)vr->Data)[18] != '2') pass = TRUE;
                                break;
                            case 'i': //ui
                            case 'I':
                                if (((char *)vr->Data)[17] != '.') pass = TRUE;
                                break;
                            case 'l': //ui
                            case 'L':
                                if (vr->Length > 19 && ((char *)vr->Data)[18] != '1') pass = TRUE;
                                break;
//                            case 'n': //un  Implicit or Explicit
//                            case 'N':
                            default:
                                if (((char *)vr->Data)[17] != '.') pass = TRUE;
                                if (vr->Length > 19 && ((char *)vr->Data)[18] != '1') pass = TRUE;
...
/*        
    int 	rc = FALSE, status = 0, compL2, size;
    signed char *p;
    BOOL	pass = false, known = TRUE ;// all;
    VR  *vr;
    UNUSED_ARGUMENT(Filename);
	...
	//added uncompressed LittleEndianImplicit, LittleEndianExplicit or BigEndianExplicit. bcb	    
		...
		if (vr && vr->Data)
			{
            s[18] = '0';// Used for switch in LittleEndianImplicit. bcb do before strncpy 20120703
			strncpy(s, (char*)vr->Data, vr->Length); s[vr->Length] = 0;
...
BOOL To8bitMonochromeOrRGB(DICOMDataObject* pDDO, int size, int *Dimx, int *Dimy, unsigned char **out, int RGBout, int level, int window, unsigned int frame)
{ int			i, j, dum;
  VR*			pVR;
  unsigned int		iMaxRowsColumns;
  unsigned short	iRows, iColumns, iNewRows, iNewColumns, iDownsizeFactor;
