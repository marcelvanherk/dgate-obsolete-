...
//BOOL MakeSafeString (VR	*vr, char *string, Database *db); //bcb moved to dgate.hpp
...
// bcb 20120703 moved to dgate.hpp
...
RecompressPatient (char *Device, char *PatientID, char *Compression, ExtendedPDU_Service *PDU)
	...
	if (iniValuePtr != NULL && iniValuePtr->sscscpPtr->ArchivePtr != NULL &&
		iniValuePtr->sscscpPtr->ArchivePtr->ConverterPtr != NULL &&
		iniValuePtr->sscscpPtr->ArchivePtr->ConverterPtr->GetSize() > 0)
		{
		if (!aDB.Query(Tables, "DICOMImages.DeviceName, DICOMImages.ObjectFile", QueryString, NULL))
			{
...
ProcessMoveDevicePatient (char *Device, char *PatientID)
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
	if (iniValuePtr->sscscpPtr->MoveDevicePtr->ConverterPtr == NULL ||
		iniValuePtr->sscscpPtr->MoveDevicePtr->ConverterPtr->GetSize() == 0) return;
...
ModifyDeviceNameForPatient (char *Device, const char *PatientID, char *UpdateString)
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
  	sprintf(Table, "%s", iniValuePtr->sscscpPtr->ImageTableName);
...
static int
MakeListOfLRUPatients(char *Device, int KB, char **PatientIDList, char *lLRUSort, ExtendedPDU_Service *PDU)
	{
	// Query the patient table, and sort it by access time
//	SQLLEN			sdword,sdword1;
//	DWORD			iTime;
...
//	char			s[255];
	...
	// if (lLRUSort[0] || 1)// bcb? or with constant?
		{
		Patients = MakeListOfOldestPatientsOnDevice(PatientIDList, KB>0 ? MAXPATIENTS : -MAXPATIENTS, Device, lLRUSort);
		KBTotal  = 0;
		...
		for (i=0; i<Patients; i++)
			...
			RecompressPatient (Device, PatientID, iniValuePtr->sscscpPtr->ArchiveCompression, PDU);
...
MakeListOfPatientsOnDevice(char *Device, char **PatientIDList)
	...
	if (DB.db_type == DT_DBASEIII)
		{
		// count #patients
		if (!DB.Query(iniValuePtr->sscscpPtr->PatientTableName, "PatientID", "", "DICOMPatients.AccessTime"))
			return -1;
		...
		if (!DB.QueryDistinct(iniValuePtr->sscscpPtr->PatientTableName, "PatientID", "", "DICOMPatients.AccessTime"))
			return -1;
		...
		*PatientIDList = (char *)malloc(256 * MAXPATIENTS);
		...
		for (i=0; i<NPat; i++)
			...
			if (!DB.Query(iniValuePtr->sscscpPtr->ImageTableName, "ImagePat", QueryString, ""))
				{
				free (PatList);
				free (PatientIDList);
				return -1;
...
BOOL RestoreMAGFlags()
	{
	unsigned int DeviceNo, aSize = 0;
	char    FromName[20], ToString[256];

	IniValue *iniValuePtr = IniValue::GetInstance();
	if(iniValuePtr->sscscpPtr->MAGDevicePtr != NULL)
		aSize = iniValuePtr->sscscpPtr->MAGDevicePtr->GetSize();
	for (DeviceNo=0; DeviceNo<aSize; DeviceNo++)
		{
...
SelectLRUForArchival(char *Device, int KB, ExtendedPDU_Service *PDU)
	...
	NPatients = MakeListOfLRUPatients(Device, KB, &PatientIDList,
		IniValue::GetInstance()->sscscpPtr->LRUSort, PDU);
	if (NPatients < 0)
		{
...
MakeListOfSeriesOnDevice(char *Device, char **SeriesList, int age, int kb)
	...
	int 			Series, MAXSERIES = 10000, Size;//, i;
	struct tm		tmbuf1;
	...
	sprintf(Tables, "%s, %s", iniValuePtr->sscscpPtr->ImageTableName, iniValuePtr->sscscpPtr->SeriesTableName);
...
MakeListOfOldestPatientsOnDevice(char **PatientIDList, int Max, const char *Device, char *Sort)
	...
	if (!DB.Query(iniValuePtr->sscscpPtr->StudyTableName, "PatientID", "", NULL))
		return -1;
	...
	if (!DB.Query(iniValuePtr->sscscpPtr->StudyTableName, s, "", NULL))
		{
		free(StudyList);
		return -1;
		}
