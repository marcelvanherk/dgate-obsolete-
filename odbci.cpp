...
20120703   bcb    Changed WIN32 to !UNIX, made lSQLStatement (local SQLStatement) in Database :: Exec.
20120710   bcb    Used ROOTCONFIG_SZ
20130226   bcb    Replaced gpps with IniValue class.  Removed all globals now in IniValue class.
                        Version to 1.4.18a.
20130416   bcb    Fix Windows default datbase dll loads.  Fixed crash on bad ini SQLServer file path.
*/
...
//static  int OdbcDoubleBackSlashToDB=0; //bcb Same as DoubleBackSlashToDB
...
{ signed char *p=(signed char *)in, *q=(signed char *)in;
  while (*p)
  { switch (*p)
    { case '\\': p++; if (*p!='%' && *p!='_' && *p!='[' && (*p!='\\' || !(IniValue::GetInstance()->sscscpPtr->DoubleBackSlashToDB)))//bcb was OdbcDoubleBackSlashToDB
                        *q++='\\'; break;			// \% \_ \[    -> % _ [
								// \\          -> \ is optional
...
{ char *p=in, *q=in;
  while (*p)
  { switch (*p)
    { case '\\': p++; if (*p!='\\' || !(IniValue::GetInstance()->sscscpPtr->DoubleBackSlashToDB))//bcb was OdbcDoubleBackSlashToDB
                        *q++='\\'; break;			// \\          -> \ is optional
      case '\'': p++; if (*p=='\'') *q++='\'', p++;  	break;	// ''	       -> '
      default  : *q++ = *p++;
    }
...
ntuples(0),
current_tuple(0),
#endif //defined(POSTGRES) || defined(USEMYSQL) || defined (USESQLITE)
Connected(FALSE),
thq(),
queryrecno(0),
hashfield(-1),
...
#endif //__GNUC__
{
	...
	if(iniValuePtr->IniReadDone())
		db_type = iniValuePtr->sscscpPtr->db_type;
	else db_type = DT_ODBC;
...
BOOL	Database :: Open (
	...
	{ 
	  ...
	  int 			dbnum=0, n;
          ...
          if (NeedPack==2) iniValuePtr->sscscpPtr->PackDBF = FALSE;     /* 20 MB index, no pack - used during full regen (please clear db first) */
          if (NeedPack==3) iniValuePtr->sscscpPtr->PackDBF = TRUE;	/* 20 MB index force pack - used for c-echo maintenance */
          if (NeedPack==4) iniValuePtr->sscscpPtr->PackDBF = FALSE;	/* 20 MB index force index - used for c-echo maintenance */
          if (NeedPack==5) iniValuePtr->sscscpPtr->PackDBF = FALSE;	/* 20 MB index force non-threaded index - used for command line maintenance */

          if (NeedPack>1 && iniValuePtr->sscscpPtr->IndexDBF<20) iniValuePtr->sscscpPtr->IndexDBF=20;	// allocate at least 20 MB during regen - (for 2.5*10^6 images)

          if (db_type == DT_DBASEIIINOINDEX)
          { iniValuePtr->sscscpPtr->IndexDBF = 0;
            iniValuePtr->sscscpPtr->PackDBF  = FALSE;
          }
	  ...
	  if (NeedPack)
		...
		dbase_handles[i]->MBExtra    = iniValuePtr->sscscpPtr->IndexDBF;
	...
	{	char *port = NULL;
		...
		strcpy(host, iniValuePtr->sscscpPtr->DataHost);	// allows host:port syntax for Postgres
		if(p=strchr(host, ':'))
		{
			port = p+1;
			*p = 0;
	...
	{	int port = MYSQL_PORT;
		...
		strcpy(host, iniValuePtr->sscscpPtr->DataHost);	// allows host:port syntax for MySQL
		if((p=strchr(host, ':')))
		{
			port = atoi(p+1);
			*p = 0;
...
BOOL	Database :: Query (	const char	*Table,
	...
	{ if (DebugLevel>=4)
	  { SystemDebug.printf("Query Tables: %.1000s\n", Table);
          ...
          return InternalQuery(Table, Columns, Where, Order, strcmp(Table, IniValue::GetInstance()->sscscpPtr->WorkListTableName)!=0);
		...
		{ r = strstr(r, fname);					// match the field name again for range comparison
	      ...
	      // try to convert normal query to an indexed one (works when Study and/or SeriesInstanceUID passed)
              ...
              { char pat[128]="";
                ...
                if (memicmp(Table, iniValuePtr->sscscpPtr->ImageTableName, strlen(iniValuePtr->sscscpPtr->ImageTableName))==0) 
                { InternalQuery (iniValuePtr->sscscpPtr->StudyTableName, "PatientID", Where, NULL, FALSE);
                  if (queryfields[0] && querymodes[0]==2)
                  { BindField (1, SQL_C_CHAR, pat, 128, &len);
                    NextRecord();
                  }
                  else
                  { InternalQuery (iniValuePtr->sscscpPtr->SeriesTableName, "SeriesPat", Where, NULL, FALSE);
                    if (queryfields[0] && querymodes[0]==2)
                    { BindField (1, SQL_C_CHAR, pat, 128, &len);
                      NextRecord();
                    }
                ...
                else if (memicmp(Table, iniValuePtr->sscscpPtr->SeriesTableName,
                                 strlen(iniValuePtr->sscscpPtr->SeriesTableName))==0) 
                { InternalQuery (iniValuePtr->sscscpPtr->StudyTableName, "PatientID", Where, NULL, FALSE);
                  if (queryfields[0] && querymodes[0]==2)
                  { BindField (1, SQL_C_CHAR, pat, 128, &len);
                    NextRecord();
...
//OperatorConsole.printf("Hashing found record in db %s: %d\n", thq->filename, queryrecno);
	    ...
	    if (queryrecno)
          ...
          IniValue *iniValuePtr = IniValue::GetInstance();
          if (t>iniValuePtr->sscscpPtr->LongQueryDBF)
          { char f[512], g[30];
            strcpy(f, "(fields:");
            ...
            if (!(iniValuePtr->sscscpPtr->IndexDBF) && usehash)
              OperatorConsole.printf("Hashed  query in %s %s took long: %d ms\n", thq->filename, f, t);
            else if (iniValuePtr->sscscpPtr->IndexDBF && (indexval || usehash))
          ...
          { if (dbase_handles[i])
            { if (stricmp(filename, dbase_handles[i]->filename)==0)
              { dbase_close(dbase_handles[i]); dbase_handles[i]=NULL;
                err = dbase_create(filename, fielddata, &dbase_handles[i]);// bcb Can fail!
                for (int j=0; j<MAXFIELDS; j++) free(fielddata[j]);
//                if (Indices[i])				/* clear in-memory index if any */
//                  IndicesCount[i]=0;
                if(err == 0)return TRUE;// bcb dbase_create can fail!
        //bcb? Report error reason here?
                return FALSE;
              }
            }
          }
          ...
          { if (!dbase_handles[i])
            { err = dbase_create(filename, fielddata, &dbase_handles[i]);// bcb Can fail!
              for (int j=0; j<MAXFIELDS; j++) free(fielddata[j]);
              if (err != 0)// Check for dbase_handles[i] not created!
              {// Should error messages be printed here?
                return FALSE;
              }
              dbase_handles[i]->n = i;// bcb My test crashed here!  Could not create filename.
//              if (Indices[i])				/* clear in-memory index if any */
//                IndicesCount[i]=0;
              return TRUE;
            }
...
BOOL	Database :: Exec (
	...
	if (dir[0])
        ...
        if (Postgres)
        {
                if (PGSQLExec((char *)lSQLStatement) == 0)
                        return TRUE;
#ifdef  DB_DEBUG
                SystemDebug.printf ( "***Failed PGSQLExec : %.1000s\n", lSQLStatement);
                PrintLastError();
        ...
        if (Mysql)
        {
                if (MYSQLExec((char *)lSQLStatement, FALSE) == 0)
                        return TRUE;
#ifdef  DB_DEBUG
                SystemDebug.printf ( "***Failed MYSQLExec : %.1000s\n", lSQLStatement);
                PrintLastError();
        ...
        if (SqLite)
        {
                if (SQLITEExec((char *)lSQLStatement, FALSE) == 0)
                        return TRUE;
#ifdef  DB_DEBUG
                SystemDebug.printf ( "***Failed SQLITEExec : %.1000s\n", lSQLStatement);
                PrintLastError();
...
#ifndef UNIX
	NewStatement();
	RetCode = SQLExecDirectWithRetry ( hStmt, (unsigned char *) lSQLStatement, SQL_NTS );
...
BOOL	Database :: CreateDatabase (
	...
	NewStatement();
	sprintf (SQLStatement, "exec sp_addlogin '%s','%s','%s'", User, IniValue::GetInstance()->sscscpPtr->Password, Name);
	i = SQLExecDirectWithRetry ( hStmt, (unsigned char *) SQLStatement, SQL_NTS );
        if (i != SQL_SUCCESS_WITH_INFO && i != SQL_SUCCESS)
		{ 
		SystemDebug.printf ( "***Failed SQLExecDirect : %.1000s\n", SQLStatement);
