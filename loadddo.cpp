...
20100717 mvh     Merged
20120703 bcb     Removed WHEDGE, changed WIN32 to !UNIX
20130331 bcb     Fix size warnings.
*/
...
class CBufferedIO
{
  private:
    ...
    size_t		m_iBufSize;
    size_t		m_iCurPos;
    size_t		m_iNbBytesInBuffer;
    size_t		m_iNbBlocksRead;
  public:
    CBufferedIO(int iBufSize = 0x4000);
    ~CBufferedIO();
    int  read (int handle, void *buffer, size_t count);//bcb ? Should be size_t?
    long lseek(int handle, long offset, int origin);//bcb ? Should be size_t?
#ifdef __GNUC__
...
int CBufferedIO::read (int handle, void *buffer, size_t count)
{ size_t		iNbBytesCopied = 0;
...
LoadImplicitLittleEndian(DICOMDataObject* pDDO, CBufferedIO* pBufferedIO,
	int handle, unsigned int iVrSizeLimit, size_t uFileSize)
{
...
LoadImplicitLittleEndianFile(char* filename, unsigned int iVrSizeLimit)
{
	...
	char			Buf[128 + 4];
	PDU_Service		PDU;
	size_t		uFileSize;//,iLastTime, iCurTime;
	...
	uFileSize = lseek(handle, 0, SEEK_END);
