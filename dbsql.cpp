...
20120703    bcb	Changes WIN32 & WINDOWS to !UNIX, removed WHEDGE, created and included dbsql.hpp
              	Moved includes, defines and prototypes to device.hpp
20120710    bcb	Use ROOTCONFIG_SZ and MAX_PATH
20120820	bcb	Fixed warnrig. Removed ACRNEMAMAP, moved init to the ACRNemaMap Class.
20130226	bcb	Removed gpps, added IniValue class.  Removed all globals now in IniValue class. Version to 1.4.18a.
20130416	bcb	Fixed worklist test to check WorkListMode.
*/
...
FixImage(DICOMDataObject *DDOPtr)
	...
	else if (vr->Length==10 && vr->Data && iniValuePtr->sscscpPtr->FixPhilips)
		{
		char patid[11];
		int Length;
	...
	else if (vr->Length==8 && vr->Data && iniValuePtr->sscscpPtr->FixKodak)
		{
		char patid[11];
		int Length;
...
SaveToDataBase(
	...
	AccessUpdate.printf("%s", s1);//bcb? AccessUpdate never turned on and ACPipeName never used.
	IniValue *iniValuePtr = IniValue::GetInstance();

//	RemoveDuplicates(DB, PatientDB, iniValuePtr->sscscpPtr->PatientTableName, DDOPtr, FALSE);
//	AddToTable(DB, PatientDB, iniValuePtr->sscscpPtr->PatientTableName, DDOPtr, NULL, NULL);
//	RemoveDuplicates(DB, StudyDB, iniValuePtr->sscscpPtr->StudyTableName, DDOPtr, FALSE);
//	AddToTable(DB, StudyDB, iniValuePtr->sscscpPtr->StudyTableName, DDOPtr, NULL, NULL);
//	RemoveDuplicates(DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, DDOPtr, FALSE);
//	AddToTable(DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, DDOPtr, NULL, NULL);
//	RemoveDuplicates(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDOPtr, FALSE);
//	AddToTable(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDOPtr, filename, device);

	if (!UpdateOrAddToTable(DB, ImageDB,  iniValuePtr->sscscpPtr->ImageTableName,   DDOPtr, filename, device, LastPatid, NULL, &AddedImage, JustAdd, FALSE))
		return FALSE;

	if (!UpdateOrAddToTable(DB, SeriesDB,  iniValuePtr->sscscpPtr->SeriesTableName,  DDOPtr, filename, NULL,   LastPatid, NULL, &AddedSeries, FALSE, FALSE))
		{
	        if (AddedImage)  RemoveDuplicates(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDOPtr, TRUE);
		return FALSE;
		}

												// check for conflicting patid -------V */
	if (!UpdateOrAddToTable(DB, StudyDB,   iniValuePtr->sscscpPtr->StudyTableName,
		DDOPtr, filename, NULL,   LastPatid, Modality, &AddedStudy, FALSE, TRUE))
		{
		if (AddedSeries) RemoveDuplicates(DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, DDOPtr, TRUE);
	        if (AddedImage)  RemoveDuplicates(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDOPtr, TRUE);
		return FALSE;
		}

	if (!UpdateOrAddToTable(DB, PatientDB, iniValuePtr->sscscpPtr->PatientTableName, DDOPtr, filename, NULL,   LastPatid, NULL, &AddedPatient, FALSE, FALSE))
		{
		if (AddedStudy)  RemoveDuplicates(DB, StudyDB, iniValuePtr->sscscpPtr->StudyTableName, DDOPtr, TRUE);
		if (AddedSeries) RemoveDuplicates(DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName, DDOPtr, TRUE);
	        if (AddedImage)  RemoveDuplicates(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDOPtr, TRUE);
		return FALSE;
		}
...
char *
SetString(VR	*vr, char	*s, UINT32	Max)//20120820 bcb Max always a constant number
	{
	int		Index;
	UINT16	*tuint16;
	UINT32	*tuint32;
			...
			switch(DCMGateDB[Index].SQLType)
				...
				case	SQL_C_CHAR:
					if (vr && DCMGateDB[Index].SQLLength < vr->Length)
						...
						strcat(name, ",");
						if (strstr(iniValuePtr->sscscpPtr->AllowTruncate, name))
...
//		OperatorConsole.printf("Skipping Cached Update on Table %s\n", TableName);
	...
	*/
	...
	if (DB.NextRecord())
		...
		if (pm1)
			{
			if (iniValuePtr->sscscpPtr->DoubleBackSlashToDB)
				{
				pTmp = TempString;
			...
			if (!flag)
				...
				if (iniValuePtr->sscscpPtr->DoubleBackSlashToDB) len++;
				memmove(Updates+pm2+len-1, Updates+pm2-1, sizeof(Updates)-pm2-len);
				Updates[pm2-1] = '\\';
				if (iniValuePtr->sscscpPtr->DoubleBackSlashToDB) Updates[pm2++]   = '\\';
...
//						VR FakeVR;//Already declared
						...
						strcpy(s[i-1], temp+1);
							...
							if ((strcmp(TableName, iniValuePtr->sscscpPtr->PatientTableName) == 0) &&
							    (strlen(s[i-1]) > 0))
								{
							...
							// check link
							else if (i==LastDBE(DCMGateDB) && strcmp(TableName, iniValuePtr->sscscpPtr->PatientTableName) != 0)
								{
...
UpdateAccessTimes(
	...
	if(!ConnectedDB.UpdateRecords(iniValuePtr->sscscpPtr->ImageTableName,s,s1))
		{
		SystemDebug.printf("***Could not update access time for Image level database\n");
		return ( FALSE );
		}
	if(!ConnectedDB.Query(iniValuePtr->sscscpPtr->ImageTableName, UniqueLink(ImageDB), s1, NULL))
		{
	...
	if(!ConnectedDB.UpdateRecords(iniValuePtr->sscscpPtr->SeriesTableName,s,s1))
		{
		SystemDebug.printf("***Could not update access time for Series level database\n");
		return ( FALSE );
		}
	if(!ConnectedDB.Query(iniValuePtr->sscscpPtr->SeriesTableName, UniqueLink(SeriesDB), s1, NULL))
		{
		SystemDebug.printf("***Could not obtain Series level link\n");
		return ( FALSE );
		}
	...
	sprintf(s, "AccessTime = %u", (unsigned int)CurTime);
	if(!ConnectedDB.UpdateRecords(iniValuePtr->sscscpPtr->StudyTableName,s,s1))
		{
		SystemDebug.printf("***Could not update access time for Study level database\n");
		return ( FALSE );
		}
	if(!ConnectedDB.Query(iniValuePtr->sscscpPtr->StudyTableName, UniqueLink(StudyDB), s1, NULL))
		{
	...
	sprintf(s, "AccessTime = %u", (unsigned int)CurTime);
	if(!ConnectedDB.UpdateRecords(iniValuePtr->sscscpPtr->PatientTableName,s,s1))
		{
...
dbGenUID(char	*oString)
	{ 
	static char dbUIDPrefix[66];//bcb Only used here, moved here.
	static BOOL dbLoadRoot = TRUE;
	if (dbLoadRoot)
		{
		strcpylim(dbUIDPrefix, IniValue::GetInstance()->sscscpPtr->UIDPrefix, 66);
...
GetFileName(VR	*SOPInstance, char	*filename, char	*device, Database &ConnectedDB, BOOL UpdateLRU, char *patid, char *study, char *series)
	...
	if (iniValuePtr->sscscpPtr->TruncateFieldNames)
		s[iniValuePtr->sscscpPtr->TruncateFieldNames] = 0;
	...
	if(!ConnectedDB.Query(iniValuePtr->sscscpPtr->ImageTableName, "ObjectFile, DeviceName", s, NULL))
	{
...
DICOMDataObject	*MakeCopy(
	...
	while (( vr = DO->Pop() ))
		...
		if (vr->SQObjectArray)
			...
			for (unsigned int j=0; j<ADDO->GetSize(); j++)// bcb unsigned
				{ 
...
NewDeleteFromDB(DICOMDataObject	*pDDO, Database	&aDB)
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
	if (!aDB.Query(iniValuePtr->sscscpPtr->ImageTableName, "DICOMImages.SopInstanc", QueryString, NULL)) return FALSE;
	aDB.BindField (1, SQL_C_CHAR, Dum, 255, &sdword);
	if (aDB.NextRecord())
		aDB.DeleteRecord(iniValuePtr->sscscpPtr->ImageTableName, DeleteString);
	...
	if (!aDB.Query(iniValuePtr->sscscpPtr->ImageTableName, "DICOMImages.SeriesInst", QueryString, NULL)) return FALSE;
	aDB.BindField (1, SQL_C_CHAR, Dum, 255, &sdword);
	if (!aDB.NextRecord())
		aDB.DeleteRecord(iniValuePtr->sscscpPtr->SeriesTableName, DeleteString);
	...
	if (!aDB.Query(iniValuePtr->sscscpPtr->SeriesTableName, "DICOMSeries.StudyInsta", QueryString, NULL)) return FALSE;
	aDB.BindField (1, SQL_C_CHAR, Dum, 255, &sdword);
	if (!aDB.NextRecord())
		aDB.DeleteRecord(iniValuePtr->sscscpPtr->StudyTableName, DeleteString);
	...
	if (!aDB.Query(iniValuePtr->sscscpPtr->StudyTableName, "DICOMStudies.PatientID", QueryString, NULL)) return FALSE;
	aDB.BindField (1, SQL_C_CHAR, Dum, 255, &sdword);
	if (!aDB.NextRecord())
		{
		aDB.DeleteRecord(iniValuePtr->sscscpPtr->PatientTableName, DeleteString);
		...
		// worklist defined ?
			...
			if (aDB.Query(iniValuePtr->sscscpPtr->WorkListTableName, "DICOMWorkList.PatientID", QueryString, NULL))
			        {
				aDB.BindField (1, SQL_C_CHAR, Dum, 255, &sdword);

				while (aDB.NextRecord())
					aDB.DeleteRecord(iniValuePtr->sscscpPtr->WorkListTableName, DeleteString);
			        }
		         }
		}
...
//	VR			*vr;
	...
	IniValue *iniValuePtr = IniValue::GetInstance();
	RemoveDuplicates(DB, ImageDB, iniValuePtr->sscscpPtr->ImageTableName, DDO, FALSE);
	...
	while ( PatientADDO.GetSize() )
		...
		if(!StudyADDO.GetSize())
			...
			RemoveDuplicates(DB, PatientDB, iniValuePtr->sscscpPtr->PatientTableName,
		...
		while(StudyADDO.GetSize())
			...
			if(!SeriesADDO.GetSize())
				...
				RemoveDuplicates(DB, StudyDB, iniValuePtr->sscscpPtr->StudyTableName,
					TempStudyDDO, FALSE);
			...
			while ( SeriesADDO.GetSize() )
				...
				if(!ImageADDO.GetSize())
					...
					RemoveDuplicates(DB, SeriesDB, iniValuePtr->sscscpPtr->SeriesTableName,
						TempSeriesDDO, FALSE);
...
BOOL //bcb? Should initalizetables:2 force the change in tables if ini WorkListMode is 0?
InitializeTables(int mode)
	...
	if (!DB.Open ( iniValuePtr->sscscpPtr->DataSource, iniValuePtr->sscscpPtr->UserName,
		...
		return ( FALSE );
            ...
            if (iniValuePtr->sscscpPtr->WorkListMode != 0 && //bcb Added a better check.
                DB.Query(iniValuePtr->sscscpPtr->WorkListTableName, "DICOMWorkList.PatientID", NULL, NULL))
	        {
	...
	if (HasWorklist || mode==2) //bcb Was !HasWorklist
		{
		fprintf(stderr, "Dropping worklist\n");
		DB.DeleteTable ( iniValuePtr->sscscpPtr->WorkListTableName );
		}

        if (mode!=2)
		{
		fprintf(stderr, "Dropping other tables\n");
		DB.DeleteTable ( iniValuePtr->sscscpPtr->PatientTableName );
		DB.DeleteTable ( iniValuePtr->sscscpPtr->StudyTableName );
		DB.DeleteTable ( iniValuePtr->sscscpPtr->SeriesTableName );
		DB.DeleteTable ( iniValuePtr->sscscpPtr->ImageTableName );
#if 0
		DB.DeleteTable ( iniValuePtr->sscscpPtr->RegisteredMOPIDTableName );
		DB.DeleteTable ( iniValuePtr->sscscpPtr->UIDToMOPIDTableName );
		DB.DeleteTable ( iniValuePtr->sscscpPtr->UIDToCDRIDTableName );
#endif
	...
	if (HasWorklist || mode==2) //bcb Was !HasWorklist
		...
		DB.CreateTable ( iniValuePtr->sscscpPtr->WorkListTableName, s );
		}
	...
	strcat(s, ", qTimeStamp   int" );
        strcat(s, ", qFlags       int" );
	strcat(s, ", qSpare       varchar(64)" );
	DB.CreateTable ( iniValuePtr->sscscpPtr->PatientTableName, s );
	...
	strcat(s, ", qTimeStamp   int" );
        strcat(s, ", qFlags       int" );
	strcat(s, ", qSpare       varchar(64)" );
	DB.CreateTable ( iniValuePtr->sscscpPtr->StudyTableName, s );
	if (mode==0)
		{
		DB.CreateIndex ( iniValuePtr->sscscpPtr->StudyTableName, "study_lnk", UniqueLink(StudyDB));
		}
	...
	strcat(s, ", qTimeStamp   int" );
        strcat(s, ", qFlags       int" );
	strcat(s, ", qSpare       varchar(64)" );
	DB.CreateTable ( iniValuePtr->sscscpPtr->SeriesTableName, s );
	if (mode==0)
		{
		DB.CreateIndex ( iniValuePtr->sscscpPtr->SeriesTableName, "series_lnk", UniqueLink(SeriesDB));
		if (strstr(s, " SeriesPat "))
			DB.CreateIndex ( iniValuePtr->sscscpPtr->SeriesTableName, "series_pat", "SeriesPat");
		}
	...
	if (iniValuePtr->sscscpPtr->MaxFieldLength==0 || iniValuePtr->sscscpPtr->MaxFieldLength>255)
	  strcat(s, ", ObjectFile  varchar(255)");
	else
	  sprintf(s+strlen(s), ", ObjectFile  varchar(%d)", iniValuePtr->sscscpPtr->MaxFieldLength);

        strcat(s, ", DeviceName  varchar(32)");

	if(!DB.CreateTable ( iniValuePtr->sscscpPtr->ImageTableName, s ))
		{
		DB.PrintLastError();
		}
	...
	if (mode==0)
		{
		DB.CreateIndex ( iniValuePtr->sscscpPtr->ImageTableName, "images_lnk", UniqueLink(ImageDB));
		if (strstr(s, " ImagePat "))
			DB.CreateIndex ( iniValuePtr->sscscpPtr->ImageTableName, "images_pat", "ImagePat");
		}

#if 0
	// Create Registered MOP Device Table
	sprintf(s, "MOPID varchar(20), MOPTotalSize varchar(20), MOPUsedSize varchar(20)");
	DB.CreateTable ( iniValuePtr->sscscpPtr->RegisteredMOPIDTableName, s );
	...
	DB.CreateTable ( iniValuePtr->sscscpPtr->UIDToMOPIDTableName, s );
	...
	DB.CreateTable ( iniValuePtr->sscscpPtr->UIDToCDRIDTableName, s );
#endif
...
ConstructAE ()
	...
	memset(vr->Data, ' ', 16);
	IniValue *iniValuePtr = IniValue::GetInstance();
	memcpy(vr->Data, iniValuePtr->sscscpPtr->MyACRNema, strnlen(iniValuePtr->sscscpPtr->MyACRNema, 16));
	return ( vr );
	}
