...
20120703    bcb Changed !WIN32 to UNIX
20130226	bcb	Replaced gpps with IniValue class, fixed strlen warnings.
                        Version to 1.4.18a.
*/
...
int Debug::printf(const char *fmt, ...)
{
  ...
  if(DescribeSource)
    sprintf(s, "[%s] ", IniValue::GetInstance()->sscscpPtr->ServerName);
  else
    s[0] = 0;
